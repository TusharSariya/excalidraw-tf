...
export const STRATA_OPTION_REGISTRY: readonly StrataOptionRegistryEntry[] = [
  ...
  {
    urlParam: "strataChainRelocate",
    optionKey: "strataChainRelocate",
    kind: "boolean",
    default: false,
    surface: "advanced",
    emitClass: "C2",
    decidedNow: false,
    note: "Post-A7 exclusive-downstream chain relocate: rigid-translates a unit plus its exclusive downstream chain vertically to shorten edges. Default off (experimental); measured (flagship preset) −4.3% routed edge length / crossings 116→113 / height unchanged. SHIPPED: 'Chain relocate' renders inside the panel's collapsed 'Advanced: extra crossing-reduction passes' <details> disclosure (with coordCascade); emitted truthy-only (C2)",
  },
  {
    ...
    emitClass: "C2",
    decidedNow: false,
    note: "A7 tie-cascade (extends coordinateRefine): net-zero fixed-point column escapes to its two-sided median + chase, adopt-or-rollback on the A7 length proxy. Default off (experimental); measured (P2 audit-config) −15.5% rendered crossings / −1.6% routed edge length / height-gated. SHIPPED: 'Coordinate cascade' renders inside the panel's collapsed 'Advanced: extra crossing-reduction passes' <details> disclosure (with chainRelocate), with a dependency hint when strataCoordinateRefine is off; emitted truthy-only (C2)",
  },
  {
