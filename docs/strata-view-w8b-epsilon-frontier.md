...
- **Termination.** The descent is structurally bounded (≤2 passes × fixed hull list × fixed per-hull candidate count), and that bound is the _sole_ termination guarantee. There is **no** well-founded per-adoption decrease: an ε-band adoption improves (pen, L1) but may raise crossings (up to baseline+δ), and a later strict-lex adoption may then lower crossings again at a (pen, L1) cost — so oscillation between ε-band and strict adoptions is possible in principle. Termination holds only because the two-pass × hulls × candidates cap is finite (this matches the corrected code comment; the earlier "well-founded quantity" claim was wrong).
- **Determinism.** Pure integer arithmetic except the single relative-mode `Math.ceil(ε × baselineCrossings)` (exact at the shipped 0.01 granularity); ε ≥ 1 is used as-is and expected to be an integer. In absolute mode (ε ≥ 1) the resolver defensively floors ε to an integer (`Math.floor`) rather than trusting every call site to have rejected a fractional value.
...
**Causality framing (codex adjudication of W8 honored).** W8's final P*RS tuple improved on all three terms, so W8 itself showed no veto; the infinite-exchange-rate property is a \_structural* comparator property whose empirical bite W8b was built to measure. Measured: the veto binds **mid-descent** — the strict rule forbids every (pen, L1)-improving step that costs even one crossing, which channels the descent to a different basin than the ε=1 rule; the ε=1 basin is the one that keeps the owner pair adjacent. The Ware-grounded position (a crossing has a finite price) is therefore operative here as a selection-dynamics effect, not as a final-tuple trade.
