...
Round 8's recommended arm: does a rendered dependency-path **highlight** at interaction time substitute for **layout-level** path readability — i.e. is _cheap layout + interaction_ (`v2+HL`) at least at parity with the _expensive layout unaided_ (`strata-I` = K=4+A7)? If yes, heavy band-geometry work (Package C) is deprioritized.
...
`strata-I+HL(α)` vs unaided `strata-I`, p50 at α=0.5: **−3.35** (P1) / **−3.19** (P2). The highlight is a large modeled win on strata too — "build the highlight" is supported _regardless_ of the layout verdict; nothing here implies abandoning strata.
...
- **(i) Crossover α ≤ 0.5 (p50): MET on both presets.** P1 holds through α=0.75, P2 through α=1. _Caveat:_ at p90, P1 fails at α=0.5 — the hard-trace tail favors strata-I unless substitution exceeds ~75%.
