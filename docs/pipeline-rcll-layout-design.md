...
| — | 2026-06-24 | **M5r · coordinated per-column re-pack (`coordRepack`) — BUILT behind a default-OFF A/B toggle, requires `straighten`; a v2-class (many-small-column) intra-container Y-shortener; measured GO on the target preset. → GO.** A refinement that runs AFTER the M5 Brandes–Köpf straightener: for each container, each column, it searches a coordinated **permutation re-pack** that re-stacks the column's leaves to shorten intra-container dataflow edges in Y. **Module:** new pure `packages/excalidraw/components/terraformPipelineRepack.ts` (`repackColumns`), applied via `applyCoordRepack` in `terraformPipelineRcllPlacement.ts` at the three straighten sites (packed columns + both lane paths), only when `coordRepack && straighten`. **Algorithm:** per column, brute-force ALL permutations (Heap's algorithm) when ≤8 leaves (exact, deterministic); above 8, a deterministic candidate set (current order + barycenter-sort with stable tiebreak + all single adjacent-swap refinements), coordinate descent over columns in fixed order, ≤8 monotonic sweeps, **NO RNG (CON-8)**. Each candidate column is placed top-down as a height+gap stack pulled toward each leaf's intra-container-neighbor barycenter, **clamped within the fixed container band (the band MUST NOT grow)**. **Never-worse refinement:** the re-packed arrangement is adopted only if ALL hold — total intra-container \|ΔY\| **strictly** decreases, CON-3 containment (band not grown), CON-4/5 non-overlap, intra-container crossing-preserving (`countCrossings` before/after, counting only `sCol!==tCol` edges within the container), CON-12 (only Y + within-column order written; X/columns never touched); otherwise the straightened placement is kept untouched. **Threading:** `coordRepack` (RcllOptions / placement ctx), `pipelineCoordRepack` (snapshot/session/URL-param `coordRepack=1` / option layers), meta echoes `rcllCoordRepack` + `pipelineCoordRepack` (post-guard) + suppression `pipelineCoordRepackSuppressed`; guard `coord-repack-needs-straighten` drops it when `straighten` is off. **Measured A/B (independently verified on the real `layoutTerraformFromSources` path; repro toggles compact=0, ancillary=1, swimlaneRise=1, reorder=1, crossingMin=1, deBand=none, rankSeparate=1, straighten=1, columnPacking=shorten, staircaseBandOverlap=1):** on **`staging-extended-localstack-v2` (target)** intra-container dataflow edge \|ΔY\| total **13930→9350 (−32.9 %)**, median **195→112 (−42.6 %)**, matched per-edge net **−4580px** (22 shorter / 8 longer / 15 unchanged), intra-container crossings **4→1**, CON-3/4/5/12 all 0 both arms, band growth 0, OFF byte-identical, 25 leaves moved — **GO**; `staging-extended-localstack` +0.6 % total intra \|ΔY\| (net +750px, 8 leaves moved, CON clean — minor non-target drift); `staging-multi-state-expanded` / `staging-localstack` 0 % no-op (no many-small-column containers). **Known limitations (honest):** (1) the crossing gate preserves **intra-container** crossings only — cross-container crossings can rise (v2 global 367→371, +4), within the feature's stated scope since cross-container Y is the closed hull-coord NO-GO; (2) the accept gate is **per-container** on each container's own intra objective, so the whole-scene intra total can drift slightly on non-target presets (the +0.6 % above) — **TODO:** a **global** accept gate (revert all re-packs if the scene-wide intra total doesn't improve) would make it strictly never-worse scene-wide. **Feasibility context (recorded so nobody re-chases the mirage):** a feasibility-ceiling probe found the unconstrained "ceilings" of 39–96 % are a **MIRAGE** — they require violating CON-3 (band growth); the realistic shippable ceiling under a fixed band is **27.2 %** on v2 (brute-force exact, small columns) and **~0 %** on single-dominant-column (`staging-extended-localstack`: one 19-leaf column) or sparse-intra-edge (`multi-state`: 2 edges) topologies — so `coordRepack` is a **v2-class (many-small-column) lever, not universal.** **Rejected alternatives (record so they aren't re-attempted):** (a) **neighbor-barycenter within-column order key (M6)** — infeasible: M6 ordering runs before coordinate assignment, so it has only a within-column rank, no provisional neighbor Y to barycenter on; (b) **DEC-5 dummy-node chaining in the straightener (`straightenWideSpan`)** — BUILT then REVERTED: net-negative on v2 (median intra \|ΔY\| 262.5→316, net +1045px span) and inert on sparser presets — it bought straightness by growing the band (the CON-3 violation the `coordRepack` gate forbids). Hash backfill pending (not yet committed at time of writing). |
| — | 2026-06-24 | **X-axis network-simplex ranker — BUILT (`pipelineColumnPacking:"shorten"`, default OFF), across 3 bisectable PRs.** Promotes the CONFIRMED-GO probe (the investigation row below) into production. **Surface:** the existing `pipelineColumnPacking` tri-state widens to a 4th enum value **`"shorten"`** (mutual-exclusion with `spread`/`compact` automatic; no new boolean). **`shorten` is a BUNDLE** — it expands (in the `applyRcllToggleGuards({...})` literal at `terraformLayoutCore.ts`) to **both** `networkSimplexRank:true` AND `columnCompact:true` (the proven additive config — NS shortens cross-column edges on the depth floor, compact pulls leaves left downstream). **Algorithm:** new pure `computeNetworkSimplexDepths` (`terraformPipelineLayoutShared.ts`) — exact Gansner TSE93 (feasible tight tree → cut values → entering/leaving pivots **to the optimum**, Bland's-rule leaving-edge selection = anti-cycling + deterministic, then `balance()`); minimizes Σ w·span with `w` = parallel-edge count; solves each weakly-connected component independently; **EXACT to optimum, never a capped/partial result** (a balance heuristic stalls −10.3 %, a sign-bug −7.3 %, both still "every edge forward" — only the true simplex is trustworthy); a safety pivot ceiling (≈4 | V | ) being hit is a **BUG** → return input unchanged. **Build site (`preparePipelineLayout`, depth-floor swap slot):** NS arm takes precedence over the experimental ALAP arm; both route through a NEW shared **`applyDepthFloorIfValid`** validator (extracted PR1, no-op refactor) that **verify-or-aborts** (every edge forward) and **DUAL-WRITES** the two live consumers — `cluster.depth` (→ `computeGlobalColumnX` pixel geometry) AND `effectiveDepthResult.depths` (→ `buildRcllModel` → `lattice.floor`, the layering axis); writing only one "ships green, changes nothing." **Observability:** `prep.networkSimplexApplied` + `nsSkipReason` (`"infeasible-fallback"` \| `"cyclic-skip"`) → scene meta `rcllNetworkSimplexApplied` / `rcllNetworkSimplexSkipReason`, milestone `M2-NS`; guard surfaces `pipelineNetworkSimplexRankConflict` when NS drops a conflicting column-axis toggle. **Guard (`terraformPipelineToggleGuards.ts`):** **⟦CORRECTED post-merge — the eng-review "NS-wins-rankSeparate" call was WRONG and is reversed.⟧** User repro on the FULL-detail config (`compact=0` + `rankSeparate=1` + `swimlaneRise=1` + reorder/crossingMin/straighten) showed `shorten` made the chart **+149 % TALLER** (h 15,270 → 38,039). Root cause: `rankSeparate` (+ M4 lane-rise) is the **dominant height lever** (it gives sibling lanes disjoint column ranges so the lanes interleave in Y, ≈ −60 % height); NS re-ranks columns to min-span which **destroys that X-disjointness**, so the lanes can no longer rise and height explodes. NS and rankSeparate are genuinely mutually-exclusive column-axis strategies and CANNOT compose. **Fix: rankSeparate WINS** (the larger lever) — when a LIVE rankSeparate (passed its `needs-rise` precondition first) and NS are both requested, NS is dropped + surfaced (`pipelineNetworkSimplexRankSuppressed`, suppression `rank-floor-conflict-rankseparate-wins-network-simplex`); milestone stays `M8r`. So `shorten` on a rankSeparate layout is a **safe no-op** (degrades to its additive `columnCompact` half, ≡ `compact`), never a regression. NS still **wins over** `deDensify`; `columnCompact` is additive, NOT guarded. NS is only effective when rankSeparate is off (the config its probe measured). Tests assert the no-regression (`shorten`+rankSeparate ≡ `compact`+rankSeparate; height NOT blown up). **Realized A/B (v2, this build, `shorten` vs `compact`, crossingMin on both):** width **strictly narrower (≥3 % gated; probe −8.4 %)**, all structural gates 0. **⚠️ Crossings correction:** the probe's "crossings flat" held only against its FULL-toggle baseline; in an isolated NS-vs-compact A/B the depth-floor re-ranking **trades a small crossings increase** (≈230 vs ≈198 with crossingMin on) for the width win — so crossings is NOT asserted as a gate (the robust, NS-attributable wins are width + clean structural gates). **Tests:** synthetic provable-optimum DAG (span 12→10, slack node pushed right — fixture-independent, catches the heuristic/sign-bug false-win class), property (span ≤ longest-path + every edge forward + layer count preserved, multi-case incl. disconnected), weighting flips the optimum, determinism ×3, dual-write invariant, cyclic-no-throw, guard rules, URL `columnPacking=shorten` round-trip, 4th UI radio, threading-reaches-engine, v2 rendered A/B. **3-PR split** (refactor / threading-with-identity-stub / exact-NS) so an algo bug and a threading bug bisect cleanly. **Why not ELK:** elkjs 0.11.1 ships `NETWORK_SIMPLEX` but only via async `elk.layout()` over the whole compound graph — no sync standalone `rank()` to extract; hand-roll justified (~430 LOC, no new dep). Eng-reviewed via `/plan-eng-review`; plan `read-and-understand-this-lovely-quokka.md`. Cross-preset validation before any profile default → [TODOS.md](../TODOS.md). |
| — | 2026-06-24 | **Edge-length minimization — investigated; two-axis reframe + Y-axis hull-aware coordinate-assignment NO-GO (realized + gate-verified). X-axis network-simplex layering = the only remaining lever (measurement pending). Research/measurement record only; no engine change.** A prior "edge length = the M5 straightener, nothing to do" answer was **wrong** and is corrected here. Edge length is **two-axis**: **(X-span)** edge length proper = layers crossed (`rank(v)−rank(u)`), owned by **layer assignment (M2)** — RCLL uses naive **longest-path (ASAP)**, computes `[LB,UB]` slack but **never uses it**; and **(Y-deviation)** ΔY straightness, owned by **coordinate assignment (M5)** — the per-leaf Brandes-Köpf straightener is **clamped inside each hull's Y band** (`terraformPipelineStraighten.ts:244–256`), which is _why_ it is a v2 no-op. Research (Gansner TSE93, GD Handbook Ch.13/Healy-Nikolov, Sander 1996, Brandes-Köpf, Dwyer VPSC GD2005 / IPSep-CoLa, Jünger-Mutzel-Spisla 2018, Nikolov-Tarassov): graphviz `dot` / ELK / dagre all **default to network-simplex layering**; longest-path is the degraded option. **The user's deeper question — hull-aware Y** (make a container's Y an _active variable_, moving whole hulls so inner resources align with cross-hull neighbors; Dwyer movable-cluster-with-containment on Sander's compound-layered structure): confirmed the **seam** is post-placement / pre-ancillary (`terraformPipelineLayoutRcll.ts:489–501`) and rigid `translateSubtreeY` exists (bidirectional), but the cascade/re-derive helpers are **downward-only** and `recomputeContainerBox` is local-coord (no seam-safe re-derive). **Measure-first — 4 throwaway probes on `staging-extended-localstack-v2` (numbers are the artifact; all deleted):** (1) free-Y census **mis-targeted** (aimed each hull at the _global resource-barycenter_ → desired shifts of thousands of px) → false NO-GO; **discarded as off-target**. (2) hull-edge census (correct target = `lattice.hullEdges` sibling-local), toggles OFF: 99 hull edges, 23 already-straight, **0/28 X-disjoint straightenable** (a _third sibling_ occupies every shared target row under model-order packing). (3) hull-edge census on the **OPTIMAL config** (Compact, All-resources, `deBandLevel=none`, **all optimizer toggles ON**): `reorder`+`crossingMin` **declutter** the shared rows → **17/29 X-disjoint straightenable**; 31.5 % straightenable (≥30 % bar) + 32.8 % reclaimable hull-edge ΔY (≥15 % bar) → **apparent GO**. (4) **realized-move validation** (greedily apply the moves on a clone — translate the smaller-subtree endpoint in Y, dry-run-revert on collision, re-derive boxes bottom-up, run real gates): 31 candidates → **17 realized** (14 conflict-skipped, because each accepted move shifts geometry so later static candidates collide live), **gates all clean** (0 containment / 0 sibling-overlap / 0 CON-12 X-drift), but realized hull-edge ΔY reduction **only 3.89 %** (94,901 → 91,208 px) vs the **pre-registered ≥15 %** → **NO-GO-on-realization.** The static census **overcounted realized by ~8×** (total ΔY is dominated by X-overlapping band-stacked pairs that need _rise_ + large cross-container spans a per-container, Y-only, single-endpoint mover cannot touch). **Decision: Y-axis hull-aware coordinate assignment = NO-GO** on RCLL/v2 — the mover machinery is safe but the win is cosmetically negligible; same density/band wall as subnet-deband / rankSeparate / option-C / M4, now proven at the **realized** level with gates. Do **not** build. **⟦CORRECTION 2026-06-24 — the 3.89 % NO-GO reached the right verdict by the wrong instrument; re-measured coupled, verdict UNCHANGED but the reason is upgraded.⟧** A user challenge ("are you using the outermost hull or the innermost? moving along Y may reduce edge length if you dig deeper") exposed that probes (2)–(4) measured **per-hull-independent / single-endpoint** moves — which, on band-tight packing, zero out at every level _independently_ and under-state the coupled freedom (the literature: an outer-only / rigid-block census optimizes a **strictly smaller feasible set** — Dwyer-Koren-Marriott IPSep-CoLa TVCG2006, Sander 1996 inner-free compound coordinate pass, ELK `SEPARATE_CHILDREN` is the named deficient baseline). 3-lane re-research (graph-layout-rag corpus + web + code/transcript forensics) confirmed correct hierarchical Y minimization needs **every container offset at every level + leaf local offsets as simultaneous coupled variables**. Re-measured with a **coupled all-level solve** (two-stage gate, pre-registered ≥15 % rendered `medianVerticalDeviationPx` + all CON gates 0, throwaway probes deleted): **Stage 0a (unconstrained coupled barycenter fixed-point ceiling)** = Σ edge ΔY **151,526 → 135 px (99.91 %)** — proving the coupled freedom is **real and enormous**, NOT the 3.89 % the independent census implied (the per-hull-independent method genuinely under-measured). **Stage 0b (constrained coupled solve, rendered space, all-level hierarchical 1D VPSC + exact single-chain PAVA cross-checked)** = **NO-GO confirmed**: no Y-only formulation achieves ≥15 % with gates 0 — best feasible-ish iterate medianΔY **271 → 1831 px (worse)**, height 5,285 → 25,713 (~5×), **12 CON-3/4/5 collisions**; the _exact_ 1D PAVA drives medianΔY→0 but at **131 collisions**. **Root-cause upgrade (the load-bearing finding):** the RCLL feasible region is **intrinsically 2D and NOT separable into a Y-only 1D problem** — topology frames are bounding boxes of their subtrees and **M4 lane-rise deliberately interleaves X-disjoint subtrees in Y** to reclaim height; a 1D total-order chain cannot encode "two frames may interleave in Y **iff** their _boxes_ are X-disjoint at every level up to their LCA," so forcing edges straight piles cross-container leaves onto shared rows → the feasibility pass shoves enclosing frames apart → footprint blows up and re-bends the edges. So the Y NO-GO now rests on the correct reason: a **Y-only coordinate pass structurally cannot realize the gain**; only a full **2D coupled cluster-containment solver** (IPSep-CoLa-style, X-disjointness-aware overlap constraints) could, which is a **separate, much larger project** that would also conflict with the X-axis ranker + compaction passes. Verdict stands: do **not** build the Y-only hull pass. (Probes clean; no tracked-source change.) The **X-axis network-simplex ranker** (replace longest-path ASAP with min-total-edge-span layer assignment, future-surfaced as a mutually-exclusive `pipelineColumnPacking:"shorten"` enum value so it can't conflict with pull-left/spread) is the **only remaining** edge-length lever — orthogonal (CON-12 _permits_ rightward-within-slack moves; the earlier "v2 column axis is saturated" deferral reason was a non-sequitur — column-packing optimizes _width_, network-simplex optimizes _span_) — and its span-reduction probe is **GO**: exact Gansner network simplex (tight tree + cut values, 5 pivots to optimality) over the 123-node / 145-edge **acyclic** cluster DAG cuts total edge span **262 → 190 (−27.5 %)**, converts **9/22 (41 %)** long edges to span-1 (max span 13 → 10), **0** forward-edge violations; slack is abundant (65/123 nodes have slack>0, total 320, median 2) so ASAP genuinely wastes it. (A balancing heuristic underestimated at −10 % / stalled in a local optimum — the **exact** network simplex is required, not the cheap pass.) This is the **first edge-length lever to clear its gate.** Per the Y-axis lesson (static potential overcounted realized ~8×), the span win — though model-level and realizable-by-construction (every edge stays ≥1 forward) — still needs a **realize-on-clone re-placement check** (rendered height/width/crossings/nearStraight/gates) before the engine is built, since network simplex can trade width for shorter edges. **Realized validation (NS ranks injected into `prep.depthResult.depths` → the swimlane `denseRank(LB)` floor; rankSeparate OFF since it occupies the same column-axis slot, other toggles ON; injection fired = 88.6 % of leaf X changed; measured on the REAL scene via `buildSceneFromBoxedTree` + `diagnosePipelineScene`): CONFIRMED-GO.** Width **11762 → 10770 (−8.4 %, NARROWER not wider** — NS cures longest-path's "wide-at-the-bottom" pathology), height +2.5 %, crossings flat (242), nearStraight 0.10 → 0.11, medianΔY 1295 → 1148 px, **ALL gates 0** (acyclicBackward/sameColumn/containment/siblingOverlap/collision). The −27.5 % model-span win **realizes** into a width-for-tiny-height trade with no readability regression — the **first fully realize-validated edge-length lever** in the campaign (contrast the Y-axis, whose 32.8 % potential collapsed to 3.89 %). **Recommendation: BUILD** the X-axis network-simplex ranker (default-OFF, surfaced as the mutually-exclusive `pipelineColumnPacking:"shorten"` enum value). Eng-review via `/plan-eng-review` (scope challenge → outside voice caught the static-census over-promise + downward-only-primitive + non-convergence risks → reframed to census-then-realize). plan `read-and-understand-this-lovely-quokka.md`. |
| — | 2026-06-22 | **Zoom-out legibility → navigation-first (Phase 1 measure + Phase 2a navigation + Phase 2b footprint-gated frame names). Default behavior preserved except an LOD-on rendering refinement; minimap is a default-OFF feature flag.** The RCLL view is illegible when zoomed out because the shipped LOD feature (`terraformLod.ts`) is **pure culling** — below per-class zoom thresholds it removes whole elements + cascade-drops edges, collapsing the scene to bare primary rectangles. CEO review (Claude + Codex, both voices) reframed "simplify the cull" as cosmetic and judged the real low-zoom job to be **navigation** (where-is-X / jump-to-cluster), which competitors serve via minimap + search-to-fit; user chose "add navigation, keep simplify." **Phase 1 (measure, no product change):** instrumented the cache-regen hot path directly (`elementCanvasRegenStats` in `renderElement.ts`, resettable COUNT as the deterministic primary metric; ms/p95 directional) + an `--suite lod` profile set in `benchmark-canvas-runtime.mjs` (lod-off / balanced / performance via the product setState path). Result on v2 all-resources, deep zoom: LOD is **modestly perf-justified** (regen 12→0, combined p95 23.4→16.8 ms, culls ~71 % of in-viewport elements) but a screenshot A/B showed **neither LOD-on nor -off is legible at ~10 %** (labels truncate either way) — confirming the reframe: keep LOD on, navigation is the real fix, placeholder-fill (the original Phase-2b cosmetic) **DEFERRED**. Decision recorded in `docs/terraform-canvas-runtime-performance.md`. **Phase 2a — navigation (PRIMARY):** (1) **search-to-fit** — `SearchMenu.tsx` already calls `scrollToContent`; net-new is forcing the jump _past the LOD cull floor_ so a matched-but-culled element renders on arrival. Pure helpers in `terraformLod.ts`: `terraformLodFloorForElement` (per-class/preset floor; `null` for always-visible classes), `terraformSearchTargetZoom` (floor wins the clamp — avoids the min>max blank-space bug), `resolveCanonicalTerraformElement` (label/icon match → canonical primary). Passed as `minZoom` so `zoomToFitBounds`'s clamp lands in-render; the separate "select the real element" LOD-bypass proved unnecessary. (2) **Minimap** (net-new, Terraform-only, behind `terraformMinimapEnabled` feature flag, default OFF) — `TerraformMinimap.tsx`: raw `fillRect`/`strokeRect` over element geometry, **never** calls `renderElement` or touches the zoom-keyed `elementWithCanvasCache` (Eng #14 — would cause the regen storm Phase 1 names); bounds cached by scene nonce; own LOD (`terraformMinimapDrawStep` strides to ≤1500 primitives); state-driven redraw via `onScrollChangeEmitter` (no idle RAF); draggable viewport rect; collapsible (localStorage); registered in `getEditorUIOffsets` so search-to-fit doesn't frame matches behind it. **Phase 2b — footprint-gated frame names (complementary; the one LOD-on default refinement):** `shouldShowTerraformPipelineFrameName` gains an optional on-screen `footprintPx` arg — topology-frame names now render once the frame's own footprint (`min(w,h)×zoom`) exceeds `TERRAFORM_FRAME_NAME_MIN_FOOTPRINT_PX` (56) instead of fixed per-role zoom floors. This does both Phase-2b goals at once: a large enclosing frame (region/VPC) keeps its name _farther out_ than the old 0.13 region floor ("alive longer"), while a small child frame stays suppressed until it no longer collides with the parent (kills the region+VPC+subnet name-stacking that naive threshold-lowering reintroduces at ~0.2). Legacy fixed-zoom path retained when no footprint is supplied (back-compat + existing tests). **Tests:** `TerraformMinimap.test.tsx` (9 — bounds Infinity/zero-area guards, aspect-ratio dims, primitive-cap stride), `terraformLod.test.ts` (+5 footprint cases: 56px threshold, region alive at 0.02, child suppressed at the stacking zoom, provider/account always, non-pipeline unaffected), `elementCanvasRegenStats.test.ts` (3 — resettable deterministic counter); typecheck + `lint:arch` clean. N5 scope-clarified above (render-time LOD ≠ layout-time scaling). **Live Chrome A/B verification of minimap sizing/collision/pan PENDING** (browser extension). plan `read-this-docs-pipeline-rcll-layout-desi-magical-acorn.md`. NOT yet committed. |
| — | 2026-06-22 | **DI-ANC-6 PR1 — read-only band-block diagnostic (allocator behavior unchanged).** The ancillary allocator now dry-runs each row-reduction breakpoint as a **candidate FINAL rectangle** (clone the Dataflow-only baseline, widen only that band so it shortens, recompute the host bottom) and runs the EXISTING gate on it, emitting a per-scope `bandBlockStatus` enum + `currentCeilingWrapWidth`/`bestValidWrapWidth`/`acceptedWrapWidth`/final band box into `rcllAncillaryAllocator.diagnostics`; the dev probe surfaces it under `ancillaryAllocator` (`blocked` subset + `statusCounts`). This proves whether a tall band is blocked and **why**, instead of letting the live `recursiveRightSlackCeiling` (which measures right-neighbor overlap against the band at its TALL pre-widen height) self-confirm a false no-op. **Finding on `staging-extended-localstack-v2`:** the `account-03 → us-west-2 → vpc-a0fcf7a066cd52312` band reports **`shared-slack-consumed`** (compact + full) — it widened but a larger within-ceiling widening exists that the greedy spent on a competing scope (contention), distinct from the synthetic ceiling-tall bug `gap-exists-current-algo-missed` the diagnostic also detects. Additive meta only: v2 band geometry is byte-identical to DI-ANC-5; the live allocation is unchanged. PR2 (the candidate-final-rectangle fix in the real greedy) is scoped, not built. DI-ANC-6; §8.1. |
| — | 2026-06-22 | **RCLL All Resources ancillary bands shipped.** The prior honest-disabled toggle is superseded: RCLL now accepts `includeAncillary`, builds provider/account/region/VPC "Unconnected" strips, and renders them as first-class `ancillaryBand` leaves. The final invariant is **post-placement insertion, not optimizer rerun**: RCLL first computes the Dataflow-only placed tree, clones that geometry, inserts measured bands at each owning hull's bottom using the hull's existing content width, expands the host, and propagates only necessary downward shifts to lower overlapping siblings. This preserves dataflow X positions and primary card sizes; Y may only grow downward where rows are inserted. Bands are excluded from dataflow collectors, layering, cyclic hull matrices, crossing minimization, `primaryClusterCount`, and primary placement metrics. The UI re-enables All Resources; meta stamps ancillary counts and zero-allocation `rcllAncillaryAllocator` placeholders. Focused v2 regression: `staging-extended-localstack-v2`, RCLL Full/compact-ish with de-band off and all geometry toggles on: All Resources does not widen, primary X/shape is stable, placement overlaps/collisions are 0. DI-ANC-4; §34.2 and commit map updated. |
...
| — | 2026-06-20 | **RCLL menu + API redesign — outcome-first "Layout" profiles (`Readable / Balanced / Compact`) with progressive disclosure; pure surface, no engine change, default byte-identical.** Follow-up to the M5c row below: after Column packing shipped, the dialog had **eight equal RCLL toggles** (Detail, Subnets, Lane split, Column packing, Ordering, Lane height, Cycle height, Straighten) phase-grouped by Sugiyama internals — the user reported it "still doesn't tell me what is going on and what depends on what," and asked for the same on the API. `/design-consultation` (not used as a brand-system author — redirected to act as the IA reviewer for this one menu) + 4 AskUserQuestion decisions: **progressive disclosure** (2 plain primary decisions, the 8 levers collapse into Advanced), **end-user-ready** framing (primary surface leads with outcomes, the `Implements`+refs dev panel moves behind the disclosure), **all three API surfaces**, **RCLL branch only**. **Single source of truth = new pure `terraformPipelineLayoutProfiles.ts`** (`resolveRcllLayoutProfile(p) → {swimlaneLaneRise, rankSeparate, subnetDeBand, staircaseBandOverlap, reorder, straighten, columnPacking}`), the ONE place a profile expands, reused by the dialog (fan-out into the 7 existing flag setters), the URL param, the TS option, and the dev endpoint. `balanced` reproduces today's exact defaults ⇒ selecting it (or no profile) is byte-identical; `readable` = stack cycles + ordering + straighten (clearest, taller); `compact` = the whole height composition (laneRise + laneSplit + subnetDeBand + cycleRise + columnPacking=compact + ordering + straighten). The profile is a **convenience layer above the existing flags** — the M5c/threading option chain + its tests are untouched; **an explicitly-set individual flag overrides the profile** (`options.pipelineX ?? pf?.x ?? default`, expanded once at the `sceneContext` literal in `layoutTerraformFromSources`). **UI (`TerraformImportPipelineSettings.tsx` RCLL branch):** primary `Detail` + `Layout` segmented controls, then a collapsed `<details>` "Fine-tune layout (advanced)" wrapping the 8 levers; touching any lever flips Layout to a non-clickable **"Custom"** badge (via 7 `setPipeline*Custom` wrappers in `useTerraformImportDialog.ts`) so the primary control never misrepresents; **Lane split is now NESTED + indented under Lane height** with an always-visible prerequisite line (`needs Lane height = Risen — off until then`), making the dependency structural instead of a silent grey-out. **API (all 3): add a high-level entry + self-describe; never rename/break existing params** (kept as advanced overrides, following the `deDensify→columnPacking` alias precedent). URL `/demo` + dev HTTP gain `profile=` plus clear aliases `laneRise↔swimlaneRise`, `laneSplit↔rankSeparate`, `cycleRise↔staircaseBandOverlap`. The dev `GET /api/terraform-layout` proof payload was **restructured from a flat `applied` bag into `{requested, resolved:{profile,flags}, applied, suppressions[], bounds, rcll, regions}`** — `suppressions[]` is the honest "why it didn't change" list, derived from the guard drops (`rankSeparate-needs-rise`, `column-packing-conflict-compact-wins`) AND measured no-ops (`columnCompactMovedCount===0 → {reason:"no-op"}`); `?describe=1` returns the param/profile catalog (self-documenting). **Live-verified on `staging-extended-localstack-v2`:** no-profile ≡ `profile=balanced` (W11762×H14285 identical); `readable` H16621 (taller); `compact` **H4936 (−65 % height)**, W15122 (wider — lane-split trades width for height, ≈ −55 % area; the help copy says so honestly rather than claiming "narrowest"); `rankSeparate` alone → `suppressions:[needs-lane-rise]`; `columnPacking=compact` alone → `suppressions:[no-op, "moved 0 columns"]`; the `laneRise/laneSplit` aliases resolve to the same engine flags. Tests: `terraformPipelineLayoutProfiles.test.ts` (7 — `balanced`==today's defaults, each bundle, purity, guard-never-violated), +3 in `terraformLayoutCoreRcllThreading.test.ts` (balanced byte-identical/no meta, compact expands+echoes+shorter, individual-flag override), +2 in `terraformDemoUrlParams.test.ts` (profile enum + alias resolution), +2 in `TerraformImportDialog.test.tsx` (primary Compact expands the bundle, advanced edit → Custom badge); typecheck + lint:arch clean (new module is pure, no core→UI import). plan `resilient-meandering-wren.md`. Committed on `rag-self-improve-campaign`. |
| — | 2026-06-20 | **De-band generalized — `subnetDeBand` boolean → `deBandLevel` hierarchy ladder (`none`/`subnet`/`vpc`/`region`/`account`/`provider`); the subnet probe lifted one level up the tree, cascading downward. Default `none` = byte-identical; the `subnetDeBand` boolean kept everywhere as the `"subnet"` alias.** User ask: "we can deband any level in the hierarchy going up starting from subnet, to vpc, to region to account to provider, all child hierarchies are debanded" (de-band VPC ⇒ subnets also de-band). The mechanism was already level-agnostic — only the role checks were hard-wired. **Engine:** `collapseSubnetsForDeBand(node)` → `collapseTreeForDeBand(node, level)`, which collapses at the level's **absorbing parent** (subnet→vpc, vpc→region, region→account, account→provider, provider→root) by replacing that node's children with **all** its descendant leaf clusters; because `collectClusterLeaves` already recurses, collapsing at the absorbing parent **cascades the entire subtree in one op** (no per-level loop). Frame suppression generalized from "skip `subnetZone`" to "skip every role with `topologyRoleDeBandRank(role) ≥ deBandLevelRank(level)`"; `topologyPathForCluster(cluster, level)` truncates to the segments strictly shallower than the target; the membership annotation now emits **one rail per dissolved level** (subnet by tier, vpc/region/account/provider by role color) with a per-level legend (`appendSubnetMembershipAnnotations(…, deBandLevel)`). Shared helpers (`DeBandLevel`, `deBandLevelRank`, `topologyRoleDeBandRank`, `DEBAND_LEVELS`) live in the pure `terraformPipelineLayoutProfiles.ts` (reused by engine + frames + annotation + surfaces). **Surface (boolean→enum + alias, ~12 files):** profiles `subnetDeBand:boolean`→`deBandLevel:DeBandLevel` (readable/balanced `none`, compact `subnet`); dialog state `pipelineDeBandLevel` + a **`<select>` "De-band depth"** (6 options) replacing the 2-button "Subnets · Boxed/De-banded"; core resolves `pipelineDeBandLevel ?? (pipelineSubnetDeBand?"subnet":undefined) ?? pf.deBandLevel ?? "none"`; URL `?deBandLevel=…` (hard-fail invalid) + `?subnetDeBand=1` alias; dev API `deBandLevel` enum + `?describe` + `resolved.flags.deBandLevel`; session/preset/sceneApply thread the enum, keep the boolean for old persisted sessions. Meta: `rcllDeBandLevel` (omit `none`) + `rcllSubnetDeBand` (true iff `"subnet"`, back-compat). **MEASURED per level (v2 Compact — the durable finding: only subnet/vpc are clean):** `subnet`/`vpc` = all gates 0, shorter, X intact (same-col 0); `region`/`account` = structurally safe (containment/sibling/backward/collision 0) but the cross-container merge reshapes columns → **same-column edges > 0** + wider/taller bounds (account ≈ +96 %); `provider` = the single root stack overflows placement → **overlaps + backward edges + collisions** (broken on a dense graph). Cause: subnet→vpc/vpc→region merge inside ONE swimlane's global-floor column axis (CON-12 by construction); `region+` merge across containers that never shared an axis, so stale per-container `localColumn` no longer forms a coherent layering. All levels ship (opt-in, default `none`), deep levels **reported honestly** (dev-API gates/bounds + menu help flag them); a future milestone could make them clean via a post-collapse leaf-granular global re-rank (the same fix `rankSeparate` round-4 used). Tests: profiles (`deBandLevel` bundles + alias), threading (`none`≡off byte-identical, `vpc` reaches engine, `subnetDeBand` alias≡`subnet`), URL (`deBandLevel=vpc` + invalid null + alias mapping), dialog (the `<select>` threads `pipelineDeBandLevel` + flips Layout→Custom), annotation (rail-per-level + subnet byte-identical), new `terraformPipelineRcllDeBand.test.ts` (per-level v2 gates + frame-suppression + measured-deep-level profile), existing `terraformPipelineSubnetDeBand.test.ts` still green via the alias. typecheck + lint:arch clean. §8.2 un-deferred + generalized; DEC-11 generalized; plan `resilient-meandering-wren.md`. NOT yet committed. |
| — | 2026-06-20 | **THREADING BUG — `rankSeparate` (+ `straighten`/`deDensify`/`staircaseBandOverlap`) were silently dropped on the real app path; FIXED + a curlable proof API added.** The user reported the shipped **Lane split** toggle "does not work" on `staging-extended-localstack-v2` — set Lane height=Risen + Lane split=On, no wider layout. Root cause: the toggle-exposure row below claimed end-to-end threading "→ `terraformLayoutCore`", but the `sceneContext` literal in **`layoutTerraformFromSources`** (the headless entry the worker + every real import funnels through) forwarded `swimlaneLaneRise`/`subnetDeBand`/`reorder` yet **omitted `pipelineRankSeparate`, `pipelineStraighten`, `pipelineDeDensify`, `pipelineStaircaseBandOverlap`** — so `ctx.pipelineRankSeparate` was always `undefined` → guard saw `rankSeparate:false` → no separation. The engine + the dialog→`terraformSceneApply` chain were correct; only the last hop dropped them. Escaped tests because `terraformPipelineRankSeparate.test.ts` calls the **builder directly**, bypassing this hop. **Fix (default-preserving, 2 edits):** (1) `terraformLayoutCore.ts` — forward the four fields in the `sceneContext` literal; (2) `terraformPlanParsing.tsx` `TerraformPlanParsingOptions` — declare them (the option type lacked them entirely). **Proof tooling:** new dev-only **`GET /api/terraform-layout`** route in `excalidraw-app/dev/terraformImportPresetDevPlugin.mjs` — runs the layout headlessly via Vite `ssrLoadModule` (jsdom + canvas-metrics + `file://` fetch shim bootstrap; `ssr.noExternal` for roughjs in `vite.config.mts`) and returns JSON (bounds, applied/suppressed toggle flags, per-region x/y). **Verified through the fixed real path (v2 Compact):** OFF w=11762/h=14285/rs=false → BOTH (rise+rankSeparate) w=**15122**/h=**8288**/rs=**true**, us-west-1 `(7658,7612)→(8154,148)` + us-east-2 `(7658,9073)→(8154,1840)` — i.e. **+28.6 % width / −42 % height**, regions pushed behind us-east-1 and risen (the documented M8r+M4 trade, now actually reachable from the UI/URL); footgun (rankSeparate w/o rise) now reports `pipelineRankSeparateSuppressed:true`. Regression test `terraformLayoutCoreRcllThreading.test.ts` (asserts meta flag forwarded + width↑/height↓ + suppression) — would have caught it; worker-parity + toggle-guard + url-param suites still green (34/34, OFF byte-identical). NOT yet committed at time of writing. |
...
| — | 2026-06-20 | **M5c · column compaction (pull-left) — the GENERALIZED mirror of M5b; BUILT behind one tri-state `Column packing` control, default OFF, measured NO-OP on v2.** User ask (after the rankSeparate GO): inside the cyclic `0002⇄0003` accounts the api6/api7 aurora DB-subnet leaves "can be moved up a column (X decreased, if Y is adjusted)" so the hull's width shrinks while it grows into height the parent already reserves — "pull left", the mirror of M5b's pull-right, asked as ONE generic algorithm. Full `plan-eng-review` ceremony (Codex adversarial correctness — 7 corrections folded; Plan design agent; `graph-layout-rag` literature: Rüegg et al. 2016 _Using One-Dimensional Compaction for Smaller Graph Drawings_, Liao–Wong 1983 constraint-graph longest-path lower bound, _Sugiyama Layouts for Prescribed Drawing Areas_). \*\*Unified both reflows under ONE opt-in enum `pipelineColumnPacking: "spread" | "none" | "compact"`** (spread = M5b deDensify, compact = M5c, none = neither; mutually exclusive by construction; default `"none"`⇒ OFF byte-identical). **Load-bearing finding (Plan agent + Codex):** compaction can't run at`buildLaneContext` (slack unknowable before frames/lane-rise) — it runs in **`arrangeSwimlaneGroup`**, the one seam with a provisional pixel measure, making the pass **measure-driven not analytic**: new pure `terraformPipelineColumnCompact.ts` (`compactColumns`) re-runs the real `arrangeSubtreeOnAxis + layoutLanesOnAxis`stack on a CLONE per trial left-move and accepts only if width doesn't grow AND no inner frame grows taller (hierarchical`fitsWithinBaseline`, reused from the packed `computePackedPullLeftShifts`). **Codex's 7 corrections all folded:** mutable current-column map (read at consideration time, not a snapshot); refuse cross-hull neighbours (absolute X unknown at this seam); parent-slack is **measured** not guessed; **pixel** height not occupancy count; lower bound from **direct** in-hull predecessors only (`max(src col)+1`); engine-level mutual exclusion with observable meta; re-dense-rank for width monotonicity. Landing is leftmost-first / multi-column; emptied columns drop + survivors re-dense-rank `0..K` (monotone ⇒ CON-12-safe — what shrinks width); final CON-12 re-verify returns the base map (`movedCount=0`) on any violation (observable safe fallback). Engine-level both-set resolves **Compact-wins** via `applyRcllToggleGuards`(new suppression`column-packing-conflict-compact-wins`) + `pipelineColumnPackingConflict` meta. Threaded the full 5-layer + import chain (`pipelineColumnPacking`: `RcllOptions`/`RcllBuildOptions`→`terraformPlanParsing`→ the`sceneContext`literal in`layoutTerraformFromSources`(the silent-drop chokepoint the threading bug above guards) → guard → URL`columnPacking`enum →`TerraformDemoAutoImport`→`useTerraformImportDialog`tri-state →`TerraformImportDialog`→`TerraformImportPipelineSettings` `Column packing: Spread / None / Compact`segmented control →`terraformPresetImport`/`terraformSceneApply`/`terraformImportSession`→ dev-API`LAYOUT_ENUM_PARAMS`path);`rcllMilestone`→`"M5c"`, `rcllColumnCompact`+`columnCompactApplied/MovedCount/ReclaimedCols/EvalCapReached`meta. **v2 result (Compact + Full): measured NO-OP —`movedCount=0`/`reclaimedCols=0`, width unchanged, CON-12 gates 0, deterministic, OFF byte-identical** (the measure vetoes every trial; same cross-container density wall as M5b / option C — the safe set is empty). **Decision: SHIP the generalized control** (correct, gated, default-OFF infra that compacts a sparser preset). Tests: `terraformPipelineColumnCompact.test.ts`(9 pure-pass units on synthetic fixtures — proves positive move + reclaim, lower-bound math, multi-step landing, cross-axis + fan-group refusal, no-source→col-0, width-veto + frame-height-veto safe fallback, determinism), 2 conflict cases in`terraformPipelineToggleGuards.test.ts`, 2 v2 integration cases in `terraformLayoutCoreRcllThreading.test.ts`(enum reaches engine = M5c, width non-increasing, gates 0, observable no-op + canonicalized byte-identity when`movedCount=0`, CON-8 determinism); 23/23 green + typecheck clean. §9.5 item 8; §9.6 (DEC-13/M5c); plan `resilient-meandering-wren.md`. NOT yet committed at time of writing. |
| — | 2026-06-21 | **M6c · container-aware crossing minimization — BUILT behind a default-OFF `crossingMin` flag; pays back the entire `rankSeparate` +45 % crossings cost at ZERO height/width cost. → GO.** Follow-on to DI-DEB-6: `rankSeparate`+M4 cut v2 Compact height −42 % but raised rendered crossings 250→362 (+45 %), explicitly deferred to "cross-container crossing-min, a separate milestone." Today's M6 `reorder` permutes only **leaves within a single column, per-container** (DI-M6-3: a measured no-op, "dominated by cross-container long edges… needs a global pass"). M6c is that global pass: it reorders **lanes / sub-hulls within their parent AND leaves within columns** (sibling slots only ⇒ contiguity by construction). Planned + reviewed via `/plan-eng-review` + Codex (verdict "do not build as written"; the floor-coordinate ordinal gate can lower its OWN count while raising the rendered one). **The codex-driven pivot — a RENDERED-coordinate, measure-driven loop** (the M5c `columnCompact` precedent): the candidate order is _proposed_ by a hierarchical Sander/Forster **barycenter** (a child sorts to the mean placed-Y of everything its descendant leaves connect to) but _accepted_ only after **re-placing a clone** if the **rendered** crossing count (`countPlacedCrossings` — center-to-center segments over the collapsed cluster edges through the SAME `segmentsCross` kernel `diagnosePipelineScene` uses, NOT the floor proxy) strictly drops and the structural gates do not regress. New pure `terraformPipelineRcllCrossingMin.ts` (`minimizeCrossings` hill-climb: place baseline → propose barycenter order → re-place → accept iff strictly fewer crossings ∧ containment/overlap ≤ baseline; bounded sweeps + eval budget; empty override ⇒ OFF byte-identical). **X is NEVER a function of order in this engine** (`colByCluster`/`localColumn`/the staircase column own it), so the global sibling-order override moves only **Y** ⇒ CON-12 untouched by construction; threaded as `PlaceCtx.orderOverride`/`LaneContext.orderOverride` honored by the 4 sort sites (forced bands, packed columns, swimlane lanes+leaves, SCC groups) via one `preferredOrder` comparator (override rank primary, model order tiebreak). `crossingMin` wins over the leaf `reorder` when both set (`applyRcllToggleGuards` → `ordering-conflict-crossing-min-wins`). Milestone `"M6c"`, `rcllCrossingMin` + `crossingMinApplied/Before/After/Moves/HeightDeltaPx/WidthDeltaPx/EvalCapReached` meta (height/width **reported but UNGATED** — the user's call; leaf+lane reorder turned out height-neutral, measured Δh=Δw=0). **MEASURED on v2 Compact (GO):** composed with **rankSeparate+M4**, rendered crossings **362→260 (−28 %), Δheight=0, Δwidth=0** (109 sibling moves; the proxy tracked 431→317) — discharges the DI-DEB-6 +45 % regression outright, below the pre-rankSeparate 250-ish baseline, for free; composed with **de-band `vpc`**, crossings **268→221 (−18 %)** (116 moves). All structural gates 0 in both modes (CON-12 backward/same-column, containment, sibling-overlap, rendered collisions); OFF byte-identical; ON deterministic ×2. Tests: `terraformPipelineRcllCrossingMin.test.ts` (12 engine units — scorer endpoint rule, barycenter determinism, accept/identity/eval-cap, contiguity, `countReordered`), `terraformPipelineCrossingMin.test.ts` (2 v2 integration — both configs, gates 0, OFF byte-identical, deterministic, real-delta recorded), 2 guard cases in `terraformPipelineToggleGuards.test.ts`; typecheck + the existing RCLL suites green. **Phase 1 = engine + scorer + 4 internal threading points + tests + measure (this change).** Phase 2 (the 18-file UI/URL/dev-API surface) is gated on this measured win and NOT yet built. **DEC-5 dummy-node routing** stays the named escalation iff order alone can't unroute a long edge — the rendered gate (full segments) dissolves the no-dummy soundness gap codex flagged for a _barycenter_ gate. §7.2(c) (rewrite); §9.5 item 9 (discharges the +45 % follow-on); DEC-6 (shared rendered scorer); DI-CCM-1 (§34.1); plan `resilient-meandering-wren.md`. Committed (with Phase 2) `83821086a`. |
| — | 2026-06-21 | **M6c · Phase 2 — `crossingMin` surfaced on the full UI / URL / dev-API surface (gated on the Phase-1 measured GO above).** With Phase 1's −28 %/−18 % win in hand, threaded `pipelineCrossingMin` through the same ~13-file import chain established by `pipelineReorder`/`rankSeparate`/`deBandLevel`: `RcllLayoutProfileFlags.crossingMin` (`false` in all three profiles ⇒ profiles stay byte-identical, no behavior change) → `terraformLayoutCore` (`LayoutSceneContext.pipelineCrossingMin`, guard input, post-guard meta echo + `pipelineOrderingConflict` backstop, default resolution) → `terraformPlanParsing`/`terraformImportSession`/`terraformPresetImport`/`terraformSceneApply` option types + call sites → `useTerraformImportDialog` (state + `setPipelineCrossingMinCustom` "custom"-marking wrapper + profile fan-out + the 3 import call sites + the return surface) → `TerraformImportDialog` → **`TerraformImportPipelineSettings` nested "Cross-container" On/Off control** (indented under Ordering, since M6c is the hierarchical superset of the leaf reorder; two `OPTION_HELP` entries `crossingmin.off`/`crossingmin.on` with the dev `implements`/refs) → `TerraformDemoAutoImport` → URL `?crossingMin=1` (`terraformDemoUrlParams`) → dev `GET /api/terraform-layout` (`crossingMin` boolean param + catalog description + `resolved.flags.crossingMin` + `applied.pipelineCrossingMin`/`pipelineOrderingConflict` + the `crossingMin*` placement metrics + an `ordering-conflict-crossing-min-wins` / crossing-min `no-op` suppression). The toggle-guard `ordering-conflict-crossing-min-wins` (crossingMin wins over reorder) now also surfaces on the URL/programmatic path via the core meta echo. Default OFF ⇒ byte-identical (`crossingMin:false` in every profile; OFF path unchanged). Tests: `terraformLayoutCoreRcllThreading.test.ts` (+2 — reaches engine through the real `layoutTerraformFromSources` path / guard supersession observable), `terraformDemoUrlParams.test.ts` (+1 round-trip), `TerraformImportDialog.test.tsx` (+1 Cross-container control threads `pipelineCrossingMin:true`), `terraformPipelineLayoutProfiles.test.ts` (+`crossingMin:false` in all bundles); typecheck + lint:arch clean (907 modules, no core→UI import). §34.3 commit-map (Phase 2 row); plan `resilient-meandering-wren.md`. Committed (with Phase 1) `83821086a`. |
| — | 2026-06-21 | **M5 · Straighten direct-leaf rise — FIXED the local placement blocker that made obvious non-VPC/direct resources unable to rise.** Root cause: inside `layoutLanesOnAxis`, swimlane sub-hull lanes were placed first, then direct leaf resources were always packed in a separate block below the last lane. That made `straighten=true` look like a no-op for direct non-VPC resources even when their shared-column X interval was disjoint from every already-placed lane. **Fix:** under `straighten=true`, lanes and direct leaves now share one deterministic occupancy/rise pass: each sub-hull lane reserves its frame rectangle; each direct leaf reserves its one shared-column card rectangle; a direct leaf chooses the earliest Y whose X interval does not overlap an already-placed item, so it can rise beside a lane, while same-column/overlapping leaves still stack below the conflicting lane. The existing Brandes–Köpf A1 pass still runs after this provisional mixed placement, but its result is clamped through the same occupancy separation so straightening cannot collide with lane boxes. **X is unchanged** (`colByCluster` / shared `columnX` remain the only source of leaf columns; lane-rise still only tightens lane frames when its own toggle is on), so CON-12 stays structurally safe; no dummy nodes or visible routing elements are introduced (DEC-5 remains the long-edge escalation). UI/help text now says Straighten "aligns and lifts direct resources" rather than claiming staging v2 is simply expected to show no visible change. Tests: focused synthetic placement regressions prove a later-column direct leaf rises beside a lane and an overlapping direct leaf stays below; v2 Full-detail compact-ish no-deband integration asserts gates 0, determinism, and `maxDepthPx` not increasing. |
...
- **N5.** Zoom/scale-based detail hiding **as a layout strategy** (explicitly rejected — see [§15](#15-alternatives-considered-whole-approach)). _Scope clarification (2026-06-22):_ N5 forbids the **top-down scaling compound** approach where the _layout_ relies on zoom to manage clutter (we require a full-detail **static** drawing the layout owns). It does **not** forbid a _render-time_ level-of-detail layer over that static drawing — which has since shipped (`terraformLod.ts`, culling below per-class zoom thresholds) and is the rendering-side complement to [EXT-8](#235-optional-extras-ext-1ext-12). The legibility complaint that LOD culling at low zoom is illegible was reframed (CEO review) as a **navigation** problem, not a layout one: the durable fix is overview minimap + search-to-fit (jump/frame by name), with LOD-driven graceful simplification (footprint-gated frame names) as the complement. See the 2026-06-22 change-log rows.
...
**Chosen (two shipped passes, both default-OFF A/B toggles, ordered narrow→wide).**

1. **M6 `reorder` — per-container barycenter** of cross-column neighbors, applied **only when it strictly reduces a measured crossing count**, else **model order** (`firstSequence`). Leaves **within a single column, per container** only.
2. **M6c `crossingMin` — container-aware crossing minimization (the global pass M6 could not be, DI-M6-3 / DI-CCM-1).** Reorders **lanes / sub-hulls within their parent AND leaves within columns** — the whole compound-tree _sibling order_, not just same-column leaves. The candidate order is _proposed_ by a hierarchical **Sander/Forster barycenter** (a child sorts to the mean placed-Y of everything its descendant leaves connect to — [R7] locality, but used as the PROPOSAL, not the gate), and _accepted_ only after **re-placing a clone** if the **rendered** crossing count strictly drops (the measure-driven gate; the M5c `columnCompact` precedent).

**Why.** Forster proves crossing _locality_ (the right scope for the proposal), but M6's per-container leaf-only metric cannot see crossings **between** containers — exactly where the post-`rankSeparate` crossings live (DI-M6-3 was a measured no-op for that reason). M6c lifts the scope to the full sibling tree. Crucially, the gate is the **rendered** count (codex review: a floor-coordinate ordinal gate can lower its own count while RAISING the rendered one), so the model the search optimizes IS — modulo arrow routing — what `diagnosePipelineScene` reports. The strict-improvement + structural-gate acceptance keeps it deterministic and prevents the instability that disabled `balance()` (§3.5).

**How.** _M6:_ barycenters from already-placed neighbor Y; polyline-aware counter ([DEC-6](#14-open-design-decisions)); accept iff strictly fewer. _M6c:_ `minimizeCrossings` (`terraformPipelineRcllCrossingMin.ts`) hill-climbs — place a baseline, propose a global barycenter sibling order, re-place a clone, accept iff `countPlacedCrossings` (center-to-center segments over the collapsed cluster edges, same `segmentsCross` kernel as the diagnostic — [DEC-6](#14-open-design-decisions)) strictly drops AND containment/overlap do not regress; bounded sweeps + eval budget; empty override ⇒ OFF byte-identical. The order is threaded as a global pre-order rank (`PlaceCtx.orderOverride`) honored by all four sort sites through one `preferredOrder` comparator. **X is never a function of order** in this engine (`colByCluster`/`localColumn`/the staircase column own it), so the override moves **only Y** ⇒ **CON-12 holds by construction**; height/width are reported but **ungated** (measured Δh=Δw=0 on v2). `crossingMin` wins over `reorder` when both set. **No dummy nodes** (DEC-5 stays off): the rendered gate counts full long-edge segments, so it cannot optimize phantom intermediate-layer events — DEC-5 escalates only if order alone can't unroute a long edge.

**Determinism.** Strict-improvement gate + stable model-order tiebreak; barycenter ties → model order; the override is a stable pre-order walk. **Complexity.** M6 `O(kH log kH)` per column; M6c `O(sweeps × placement)` (each sweep one full re-place; bounded by the eval budget, CON-8).
...
### 8.1 RCLL ancillary ("All resources") — reserved band + slack allocator (BUILT)

> **Status: BUILT, reserved-band path with recursive slack allocation + read-only block diagnostic** (2026-06-22; DI-ANC-1..6, [§34.2](#342-implemented-vs-specified-delta-as-built-m3a-hardening)). RCLL now honors the **All resources** toggle: unconnected resources render as per-scope "Unconnected" strips in first-class `ancillaryBand` leaves. The bands reserve bottom-row space in their owning hull, may widen only into already-empty rightward slack within the Dataflow-only root width, add height only when rows are inserted, and are excluded from dataflow leaf/model accounting. A read-only diagnostic (DI-ANC-6) now reports, per scope, **why** a tall band did or did not widen (the live greedy's behavior is unchanged).

**The target.** When "All resources" is on, render the unconnected resources as per-scope **"Unconnected" strips** scoped to the deepest trustworthy hull: VPC when `vpcId` exists, else region when known, else account when known, else provider. The existing `buildAncillaryStrips` / `layoutAncillaryStrip` / `ancillaryStripAsPseudoCluster` primitives (in `terraformPipelineLayoutAncillary.ts` / `terraformPipelineLayoutShared.ts`) produce the strip skeleton + dims; RCLL adds placement-tree reservation.
...
**As-built — post-placement reserved bottom band.** `RcllTopologyRole` includes `"ancillaryBand"`. A band node is a leaf carrying an `AncillaryStrip`, not a `PipelineCluster`. `buildTerraformPipelineRcllExcalidrawScene` first runs dataflow-only RCLL placement and treats that placed tree as the baseline. When All Resources is selected, it clones the placed tree, inserts measured band leaves under matching provider/account/region/VPC container keys at the bottom of each host hull's existing content bbox, expands the host, and propagates only necessary **downward** shifts to lower overlapping sibling hulls. It does **not** rerun layering, crossing minimization, column compaction, straightening, or normal placement after bands are inserted. The preserved invariant is: primary dataflow X positions and frame dimensions remain unchanged; Y may only grow downward where inserted rows require space. In `sizeAndArrange`, any `ancillaryBand` kids are still split out if a future pre-placement caller creates them, measured with `measureAncillaryStrip`, and placed below normal content; they are not column members and not `primaryCluster`s, so `primaryClusterCount` stays clean.

**Export / parenting.** `buildSceneFromBoxedTree` renders each band via `layoutAncillaryStrip` and feeds an export-only pseudo-cluster to `buildCompoundFramesFromLayoutBoxes` / `assignCompoundEdgeFrameParents`, so the strip frame parents under the intended topology hull. The pseudo-cluster carries `ancillaryScopeRole`, and topology grouping honors that role so account/provider strips attach directly to account/provider frames instead of synthetic `unknown-region` descendants.

**Allocator status.** The outside-in width allocator now spends only already-empty rightward slack. Each strip starts from its host hull's normal child bbox width. The allocator computes a recursive right-growth ceiling through host → parent → ancestor: every level is capped by the Dataflow-only root right edge, parent padding, and the nearest right sibling whose Y span overlaps. It never shifts existing dataflow/sibling geometry horizontally. Candidate widths are exact row-wrap breakpoints from ordered ancillary card widths; candidates are accepted only when `measureAncillaryStrip` reports a height reduction. Greedy choice is largest row-height reduction, then larger card count, then stable `scopeKey`. After each accepted allocation the simulated ancillary tree is rebuilt so ancestor slack is recalculated against the new hull footprint.

**Final safety gate.** The final All Resources tree is rebuilt from the original Dataflow-only placed tree using the accepted width map. It is accepted only if root width stays `<=` the Dataflow-only root width, every primary cluster keeps `x`, `width`, `height`, and `angle` unchanged with `y` only moving downward, and placement containment/sibling-overlap metrics stay zero. If the gate fails, allocations are dropped by lowest measured benefit until it passes; if all are dropped, the baseline reserved-band insertion remains. `rcllAncillaryAllocator` reports deterministic `widenedHullCount`, `allocatedWidthPx`, `rowSavings`, `fallbackDropCount`, `allocationCount`, and a compact per-strip summary when allocations exist.

**Block diagnostic (DI-ANC-6, read-only — does NOT change allocation).** A persistent question on dense graphs is whether a band that stays **tall and narrow** is at its true ceiling or whether the greedy left a movement-free gap on the table. `recursiveRightSlackCeiling` can't answer that honestly because it measures a right neighbor's overlap against the band at its **tall, pre-widen height** — a widening that **shortens** the band (fewer rows ⇒ higher host bottom) can clear a neighbor that only sat beside the tall band, but the live ceiling never tries it (a "false no-op"). So `diagnoseRcllAncillaryBands` runs a separate **dry-run** per scope: for each row-reduction breakpoint it materializes a **candidate FINAL rectangle** — clone the Dataflow-only baseline, widen only that band (so it shortens and the host bottom is recomputed), and run the **same** `validateAncillaryTreeAgainstBaseline` gate (factored out as `classifyAncillaryCandidate`, the single source of truth). It records, per scope, a multi-cause `bandBlockStatus` enum — `no-breakpoint` / `gap-exists-current-algo-missed` / `all-candidates-fail-validation` / `root-capped` / `ancestor-capped` / `shared-slack-consumed` / `served-best-achievable` (a boolean would lie) — plus `currentCeilingWrapWidth`, `rootLimitWrapWidth`, `bestValidWrapWidth`, `acceptedWrapWidth`, and the final band box, into `rcllAncillaryAllocator.diagnostics`. The candidate list is bounded (cap 24, keeping the gentlest + widest) so deep bands stay cheap. The dev probe (`/api/terraform-layout`) surfaces this under `ancillaryAllocator` with a `statusCounts` tally and a `blocked` subset. **It clones throughout and never mutates the live allocation — v2 band geometry is byte-identical to DI-ANC-5.**

**Finding (v2, `staging-extended-localstack-v2`).** The reported tall band `account-03 → us-west-2 → vpc-a0fcf7a066cd52312` reports **`shared-slack-consumed`** (Compact + Full): it _did_ widen, but a larger within-ceiling widening exists that the greedy spent on a competing scope. So the v2 symptom is **slack contention between sibling scopes**, distinct from the ceiling-tall `gap-exists-current-algo-missed` bug the diagnostic also detects (proven by a synthetic fixture). PR2 (the candidate-final-rectangle fix promoted into the real greedy + arbitration of shared ancestor slack, scoped, not built) is now gated on this evidence rather than on the original assumption.

**Interaction points handled** (Codex, 2026-06-18) — these are why this was a placement-engine milestone, not a "compose helpers" change:

- **Role-blindness.** `collectClusterLeaves` and placement metrics count only `role === "primaryCluster"`; `ancillaryBand` carries no `cluster`, so cyclic lane collection, fan sets, crossing minimization, straightening, and `primaryClusterCount` ignore it.
- **Injection timing.** Bands are inserted after the single dataflow placement pass, into a clone of the already-placed tree. This prevents All Resources from changing the dataflow optimizer's order, columns, widths, or primary dimensions.
- **Determinism.** Band insertion sorts children by `(minDescendantSequence, key)`, with `MAX_SAFE_INTEGER` sentinel isolated to the non-dataflow band role.
- **Mandatory band-width cap.** Each band wraps at the baseline host content bbox width; it is placed below normal content and contributes height to the host footprint.
- **Empty `normalKids`.** `sizeAndArrange` places a band at the container content origin when no normal bbox exists.

**Gate:** collision 0 target (Compact + Full, rendered `diagnostics.collisionCount` + `placementMeta`); `rcllModel.primaryClusterCount` identical ON vs OFF; primary dataflow X and frame dimensions unchanged; frame-parent chains correct (each card frame → strip "Unconnected" frame → provider/account/region/VPC hull frame); determinism ×2; OFF byte-identical to the dataflow-only baseline.

### 8.2 De-band — dissolve a hierarchy level (+ all deeper), annotate membership (BUILT, default OFF)

> **Status: BUILT, behind `deBandLevel` (`none`|`subnet`|`vpc`|`region`|`account`|`provider`), default `none`** (subnet probe 2026-06-19; generalized to the full ladder 2026-06-20; [DEC-11](#14-open-design-decisions), [§34.1](#341-implementation-decision-log-di--per-milestone-as-built) DI-DEB-1..3). The **first lever to materially reduce v2's height** after the structure-preserving levers all capped at noise ([§9.5](#95-as-built--measured-v2-2026-06-18); M4 −2 %, de-density 0, junctions 0). It changes _what is drawn_ — the direction option-C's terminal finding named. The legacy boolean `subnetDeBand` is kept everywhere as an alias for `deBandLevel: "subnet"`.
...
**Measured (v2, `subnetDeBand` ON vs OFF, real `rcllStageMeta.placement.maxDepthPx`):** height **14 374 → 10 147 (−29.4 %) Compact / 27 763 → 20 091 (−27.6 %) Full**; width −0.5 %; containment 0, siblingOverlap 0, `acyclicBackwardEdges`/`acyclicSameColumnEdges` 0; rendered collisions **12 → 0** (the 12 were the overlapping subnet frames before suppression); deterministic ×2; OFF byte-identical. `rcllMilestone` → `"M7s"`, `rcllSubnetDeBand` meta. Dialog "De-band depth" select (RCLL-only, default None) + URL `?deBandLevel=…` (alias `?subnetDeBand=1`).

**Generalization to the full ladder (2026-06-20).** The mechanism is level-agnostic: de-band at level **L** collapses at L's **absorbing parent** (the role one level shallower: subnet→vpc, vpc→region, region→account, account→provider, provider→root) by replacing that node's children with **all** of its descendant leaf clusters. Because `collectClusterLeaves` already recurses to every descendant leaf, collapsing at the absorbing parent **cascades the whole subtree in one op** — no per-level loop. Frame suppression generalizes from "skip `subnetZone`" to "skip every role with depth ≥ L" (`topologyRoleDeBandRank(role) >= deBandLevelRank(L)`); the path truncates to the segments strictly shallower than L; the annotation emits **one rail per dissolved level** (subnet by tier, vpc/region/account/provider by role color) with a per-level legend. The collapse function is `collapseTreeForDeBand(node, level)`; `none` is the no-op identity (byte-identical to today).

**Measured per level (v2 Compact, the durable finding — only subnet/vpc are clean):**

| `deBandLevel` | gates (containment / sibling / backward / collision) | same-column edges | bounds vs boxed | verdict |
| --- | --- | --- | --- | --- |
| `subnet` | 0 / 0 / 0 / 0 | 0 | −29 % height, X intact | **clean win** |
| `vpc` | 0 / 0 / 0 / 0 | 0 | shorter, X intact | **clean win** |
| `region` | 0 / 0 / 0 / 0 | **> 0** | reshaped (wider) | safe, legibility trade |
| `account` | 0 / 0 / 0 / 0 | **> 0** | ~+96 % width/height | safe, regression |
| `provider` | **overlaps + backward + collisions** | > 0 | one giant stack | **broken on dense graphs** |

The reason: subnet→vpc and vpc→region merge **within one swimlane's global-floor column axis** (X stays a longest-path layering ⇒ CON-12 by construction). `region+` merge leaves **across** containers that never shared a column axis, so the stale per-container `localColumn` no longer forms a coherent single layering — same-column edges appear, then (provider) the single root stack overflows placement entirely. All levels are kept in the ladder (user-requested, opt-in, default `none`); the deep levels are **reported honestly** (the dev API `/api/terraform-layout` gates + `bounds` surface the regression per build, and the menu help flags it). A future milestone could make the deep levels clean by **re-layering the merged set on a fresh global-floor axis** after the collapse — deferred.

**Deferred.** Re-layering the merged set so `region`/`account`/`provider` become CON-12-clean (not just structurally safe); a richer set-membership boundary (BubbleSets / LineSets / Kelp low-opacity hull) instead of the rail.
...
8. **M5c · column compaction (pull-left) — the GENERALIZED mirror of M5b de-density; BUILT + measured NO-OP on v2 (2026-06-20).** Where M5b `deDensify` pulls a free leaf **right** to de-crowd a column (spend width to make Y-room for the straightener), M5c `columnCompact` pulls a free leaf **left** into an earlier column's vertical whitespace — **spend hull height** (which the parent already reserves) to **shrink hull width**. The two are unified under ONE opt-in tri-state `pipelineColumnPacking: "spread" | "none" | "compact"` (spread = M5b, compact = M5c, none = neither; mutually exclusive by construction, default `"none"` ⇒ OFF byte-identical). The user's motivating case: inside the cyclic `0002⇄0003` accounts (e.g. the api6/api7 aurora DB-subnets) independent leaves that _could_ slide into an earlier column never do — the same gap fan-out pinning (T4) and the packed `pullLeft` both skip on the swimlane path. **Load-bearing architecture finding:** compaction **cannot** run at `buildLaneContext` (columns precede frames / lane-rise / parent bbox — slack is unknowable there), so unlike M5b it runs in **`arrangeSwimlaneGroup`** (`terraformPipelineRcllPlacement.ts`), the one seam that already produces a provisional pixel measure (`buildLaneContext → arrangeSubtreeOnAxis → layoutLanesOnAxis → {right, bottom}`). The new pure pass `terraformPipelineColumnCompact.ts` (`compactColumns`) is therefore **measure-driven, not analytic**: per candidate left-move it re-runs that real stack on a CLONE of the member set against a trial column map and accepts only if `width ≤ baseline.width` **and** no inner frame grows taller (the hierarchical `fitsWithinBaseline` per-frame height test, reused from the packed `computePackedPullLeftShifts`). Eligibility mirrors deDensify's safe rules (in-axis only, purely-forward neighbourhood, no fan-group split via the **exported** `sharesFanWithColumnMate`); the lower bound is `max(direct in-hull source col)+1` (no-source leaf may reach col 0); landing is **leftmost-first / multi-column** (full reclaim — Rüegg et al. 2016 bidirectional 1D compaction, Liao–Wong 1983 constraint-graph longest path); after the sweeps emptied columns are dropped and survivors **re-dense-ranked** `0..K` (monotone ⇒ CON-12-safe, this is what shrinks width); a final CON-12 re-verify returns the **base map untouched** (`movedCount=0`) on any violation — observable safe fallback. Meta surfaces `columnCompactApplied/MovedCount/ReclaimedCols/EvalCapReached` on `rcllStageMeta.placement`. **An engine-level both-set (`deDensify ∧ columnCompact`) resolves to Compact-wins** via the kept `applyRcllToggleGuards` (new suppression `column-packing-conflict-compact-wins`), so the enum is the single source of truth and the conflict is structurally impossible from the UI/URL. **v2 result (Compact + Full): measured NO-OP — `movedCount=0`, `reclaimedCols=0`, width unchanged, CON-12 gates 0, deterministic, OFF byte-identical** (the safe-fallback path: the measure vetoes every trial). **Why:** the same density wall as M5b / option C — v2's swimlane SCC groups are cross-container-dominated, so a left-move either hits a cross-axis neighbour (refused) or grows another frame (vetoed by the oracle); the safe set is empty. **Decision: SHIP the generalized control** (correct, gated, default-OFF infrastructure that would compact a sparser preset — its synthetic-fixture unit tests prove a positive move + reclaim); the v2 no-op is recorded, not a regression. Threaded the full 5-layer + import chain (`pipelineColumnPacking`: types → `sceneContext` literal → dialog tri-state `Column packing: Spread / None / Compact` → URL `columnPacking` → dev API enum path). DEC-13/M5c; plan `resilient-meandering-wren.md`.

9. **M6c · container-aware crossing minimization — BUILT + measured GO on v2 (2026-06-21); discharges the `rankSeparate` +45 % crossings follow-on at ZERO height/width cost.** Item 8 of the round-4 GO row (and DI-DEB-6) left ONE recorded debt: `rankSeparate`+M4 bought −42 % height but raised rendered crossings **250→362 (+45 %)**, deferred to "a cross-container crossing-min milestone." M6 `reorder` could not pay it (DI-M6-3: leaf-only/per-container ⇒ a measured no-op against cross-container crossings). M6c is the global pass — it reorders **lanes/sub-hulls within their parent AND leaves within columns** (the whole sibling tree), proposing orders by a Sander/Forster barycenter but **accepting on the RENDERED crossing count** by re-placing a clone (`terraformPipelineRcllCrossingMin.ts` `minimizeCrossings`; the M5c measure-driven precedent). The codex-driven pivot: a floor-coordinate ordinal gate can lower its OWN count while raising the rendered one, so the gate reads placed `box.x/box.y` (`countPlacedCrossings`, the same `segmentsCross` kernel as `diagnosePipelineScene`), never the floor proxy. **X is never a function of order** (the override moves only Y) ⇒ CON-12 holds by construction; height/width are reported but ungated (the user's call). **v2 Compact measurement (GO):** composed with **rankSeparate+M4**, crossings **362→260 (−28 %), Δheight=0, Δwidth=0** (109 sibling moves; proxy 431→317) — the +45 % regression is paid back outright, below the pre-`rankSeparate` ~250 baseline, **for free**; composed with **de-band `vpc`**, crossings **268→221 (−18 %)** (116 moves). All structural gates 0 both modes; OFF byte-identical; ON deterministic. The measured Δh=Δw=0 confirms the user's thesis that leaf+lane reorder is height-neutral (codex's height-budget gate, #6, was unnecessary — rejected). Locked by `terraformPipelineRcllCrossingMin.test.ts` (12 engine units) + `terraformPipelineCrossingMin.test.ts` (2 v2 integration). **Phase 1 (engine + scorer + tests + measure); Phase 2 (the ~13-file UI/URL/dev-API surface — nested "Cross-container" dialog control, `?crossingMin=1`, dev-API param) BUILT on the strength of this win, default OFF / byte-identical.** DEC-6 (shared rendered scorer); DI-CCM-1 (§34.1); §7.2(c); plan `resilient-meandering-wren.md`.
...
- **As-built (2026-06-21, M6c):** the polyline-aware crossing kernel `segmentsCross`/`Seg` in `terraformPipelineCollisionDiagnostics.ts` is now **exported and shared** (DRY) — `diagnosePipelineScene` counts crossings over arrow polylines for the final readability metric, and M6c's `countPlacedCrossings` (`terraformPipelineRcllCrossingMin.ts`) counts crossings over **box-center segments** of the collapsed cluster edges through the SAME kernel. This makes the M6c search gate a faithful box-coordinate analogue of the rendered diagnostic (the codex requirement: gate on what is drawn, not a floor proxy). The two scorers differ only in their segment source (full arrow routing vs straight box-center chord), an intentional, documented proxy gap — the v2 integration measured the real rendered count dropping _past_ the proxy margin (362→260 vs proxy 431→317), so the proxy is a sound lower-confidence guide.
...
### DEC-11 — De-band: dissolve a hierarchy level (+ all deeper) vs keep boxed containers

A VPC's subnets are drawn as stacked boxed Y-lanes, so `height(VPC) ≈ Σ(subnet bands)` — the dominant remaining height cost on a swimlane-heavy preset ([§8.2](#82-de-band--dissolve-a-hierarchy-level--all-deeper-annotate-membership-built-default-off)). Should a container tier keep its own box, or dissolve into its parent? And **at which level** — just subnets, or any level up the ladder?

- **(A) Keep boxed containers (default).** Cleanest ownership; tallest. Each `subnetZone` / vpc / region / … frame is a real containment box; resources stack within their band.
- **(B) De-band + per-card rail(s) + legend (recommended, BUILT default-OFF).** Merge the dissolved level's resources into one shared column stack; show membership as a colored rail per dissolved level + a per-level legend; suppress the dissolved frames. Non-overlap moves from box level to resource level; X untouched **at the subnet/vpc levels** (CON-12-safe by construction).
- **(C) De-band + overlapping dotted box.** The user's first idea; **rejected for the default** by the corpus (MapSets / BubbleSets disfavour overlapping group regions) — recorded as a future option if a boundary is wanted (a low-opacity hull / Kelp path, gated on an overlap metric).
- **Generalized to a depth ladder (2026-06-20).** `deBandLevel ∈ {none, subnet, vpc, region, account, provider}` replaces the original `subnetDeBand` boolean (kept as the `"subnet"` alias). De-banding a level dissolves it **and every deeper level** (cascade down), by collapsing at the absorbing parent (`collapseTreeForDeBand`). **Measured:** subnet/vpc are clean wins (all gates 0, shorter, X intact); `region`/`account` stay structurally safe but reshape the columns (same-column edges + wider bounds) because the merge crosses container column axes; `provider` overflows the single root stack into overlaps + backward edges on a dense graph. All levels ship (opt-in, default `none`); the deep-level trade is **reported honestly** (dev-API gates/bounds + menu help). Making the deep levels CON-12-clean needs a post-collapse leaf-granular global re-rank (the same fix DEC-12 round-4 used for `rankSeparate`) — deferred.
- **Status: (B) IMPLEMENTED as the `deBandLevel` ladder, default `none` (DI-DEB-1..3, generalized DI-DEB-7); (A) is the default; (C) deferred.** Phase-0 probe gated the subnet build on a measured real-height win (−29.4 %/−27.6 %); the generalization is mechanism-complete with deep levels honestly flagged as regressions. **Reversal cost:** low — `deBandLevel` is a single dial; `none` is byte-identical to (A), and `subnetDeBand=1` still resolves to `"subnet"`.
...
| — | **▸ M-ANC — RCLL ancillary ("All resources"): reserved bottom band** (BUILT; independent of M5–M12). Render the unconnected resources RCLL omitted. Recipe + rejected dead ends are in [§8.1](#81-rcll-ancillary-all-resources--reserved-band-built); DI-ANC-1..4; §34.2. | a new `ancillaryBand` role inserted after dataflow placement as a bottom band under provider/account/region/VPC hulls | accept Y-growth (a band makes a container taller); wrap at existing host content width; preserve dataflow X/shape and only push lower overlapping hulls down | **collision 0** + `primaryClusterCount` unchanged vs OFF + primary dataflow X/shape unchanged + determinism |
...
| **DI-ANC-1** | Ancillary | **Superseded:** the UI-only disabled RCLL "All resources" state was removed when reserved bands shipped. The Resources control now behaves the same under RCLL as other pipeline layouts and forwards `pipelineIncludeAncillary:true`. | The old disabled state was correct only while RCLL omitted ancillary resources. | superseded by DI-ANC-4 |
...
| **DI-ANC-3** | Ancillary | **Superseded:** the reserved-band feature is no longer deferred. The interaction points identified here became the implementation checklist for DI-ANC-4. | The earlier scope decision changed: RCLL now renders ancillary resources. | superseded by DI-ANC-4 |
| **DI-ANC-4** | Ancillary | **Built: baseline post-placement reserved bands.** `RcllTopologyRole` gains `ancillaryBand`; RCLL runs a Dataflow-only placement pass, clones that placed tree, inserts measured band leaves under provider/account/region/VPC hulls, expands hosts, and pushes only lower overlapping sibling hulls downward. It does not rerun the dataflow optimizer after insertion. Strips render through `layoutAncillaryStrip`, and `ancillaryScopeRole` pseudo-clusters preserve frame parenting. The UI enables All Resources again. | Building the reserved-band primitive resolves the no-op UI while preserving the Dataflow-only layout: primary X positions and frame dimensions remain unchanged; only Y may grow downward where inserted rows require space. Bands stay out of dataflow collectors and primary counts. | superseded by DI-ANC-5 for allocator policy |
| **DI-ANC-5** | Ancillary | **Built: recursive ancillary slack allocator.** Before final insertion, RCLL greedily widens strips into already-empty rightward slack through the host→ancestor chain. The ceiling is capped by the Dataflow-only root right edge, parent padding, and overlapping right siblings at each level. Candidate wrap widths are exact card-row breakpoints and are accepted only when measured strip height drops; ties are deterministic by card count then `scopeKey`. The final tree is rebuilt from the original Dataflow-only baseline and gated on root width, primary X/shape preservation, primary Y monotonicity, containment, and sibling-overlap. Failed gates drop allocations by lowest benefit. Meta now reports `widenedHullCount`, `allocatedWidthPx`, `rowSavings`, `fallbackDropCount`, `allocationCount`, and optional per-strip summaries. | This spends only slack the Dataflow-only diagram already had, so All Resources can reduce ancillary rows without making the chart wider or re-running the dataflow optimizer. The invariant remains post-placement insertion: no ancillary participates in layering, ordering, crossing minimization, straightening, `primaryClusterCount`, or normal placement. | updates §8.1 / §34.2 |
| **DI-ANC-6** | Ancillary | **Built (PR1): read-only band-block diagnostic; allocator behavior unchanged.** For every ancillary scope the allocator now runs a dry-run that materializes each row-reduction breakpoint as a **candidate FINAL rectangle** (clone the Dataflow-only baseline, widen only that band — which shortens it, recomputing the host bottom) and runs the EXISTING gate on it. It emits a per-scope `bandBlockStatus` enum (`no-breakpoint` / `gap-exists-current-algo-missed` / `all-candidates-fail-validation` / `root-capped` / `ancestor-capped` / `shared-slack-consumed` / `served-best-achievable`) plus `currentCeilingWrapWidth`, `bestValidWrapWidth`, `acceptedWrapWidth`, and the final band box into `rcllAncillaryAllocator.diagnostics`; the dev probe (`/api/terraform-layout`) surfaces it under `ancillaryAllocator`. **Finding:** on `staging-extended-localstack-v2` the `account-03 → us-west-2 → vpc-a0fcf7a066cd52312` band reports **`shared-slack-consumed`** (compact + full) — it widened, but a larger within-ceiling widening exists that the greedy spent on a competing scope; this is contention, distinct from the synthetic ceiling-tall bug (`gap-exists-current-algo-missed`), which the diagnostic also detects. A boolean would lie (the live ceiling self-confirms), so the enum names the actual cause. | Diagnose before fixing: the live `recursiveRightSlackCeiling` measures right-neighbor overlap against the band at its TALL pre-widen height, so a widening that shortens the band (freeing a collision-safe, movement-free gap) is never tried. PR1 proves whether/why each band is blocked from real data without touching geometry (byte-identical v2 allocations; additive meta only). | PR2 (scoped, not built): promote the candidate-final-rectangle eval to the real greedy and stop abandoning slack on the first gate failure. updates §8.1 |
...
| **DI-M5-7** | M5 | **Straighten now removes a local placement blocker: direct leaves may rise beside swimlane lanes when their X intervals do not overlap.** `layoutLanesOnAxis` used to place sub-hull lanes first and pack all direct leaf resources below the lane block, so an obvious legal direct resource in a later column could not rise. Under `straighten=true`, the swimlane-axis path now uses mixed occupancy: lane boxes and one-column direct-leaf boxes share the same rise stack; the subsequent BK Y result is clamped through that occupancy set. | Sugiyama coordinate assignment should not preserve empty Y bands reserved by an earlier ordering/packing phase; Sander-style compound layout treats sub-hulls and leaves as boxes with separation constraints. This is still Y-only: `colByCluster`/`columnX` own X, no dummy vertices are emitted, and same-column/overlapping leaves still stack. | **Built in this change.** CON-12 remains structurally safe; collision/containment gates stay 0. DEC-5 dummy/proper-layer handling remains the escalation for long-edge straightness. |
...
| **DI-DEB-7** | De-band ladder | **Generalize the subnet-only probe (DI-DEB-1..3) to a `deBandLevel` hierarchy ladder** (`none`/`subnet`/`vpc`/`region`/`account`/`provider`) — the "extend up a level" DI-DEB-1 named as revisitable. `collapseSubnetsForDeBand(node)` → `collapseTreeForDeBand(node, level)` collapsing at the level's **absorbing parent** (subnet→vpc … provider→root) by replacing its children with **all** descendant leaf clusters; the cascade is automatic because `collectClusterLeaves` already recurses. Frame suppression + path truncation + the membership rail all generalized by depth-rank (`deBandLevelRank`/`topologyRoleDeBandRank` in the pure profiles module); **one rail per dissolved level**. Surface threaded boolean→enum (`pipelineDeBandLevel`) end-to-end with `subnetDeBand` kept as the `"subnet"` alias; dialog `<select>`; URL `?deBandLevel=`; dev-API enum + `resolved.flags.deBandLevel`. Default `none` byte-identical. | The mechanism was already level-agnostic — only the role checks were hard-wired — so the user's "de-band any level, child hierarchies cascade" reduces to "collapse at the absorbing parent" + a depth-rank comparison reused by the four call sites. Keeping the boolean as an alias preserves every existing assertion/URL (the `deDensify→columnPacking` precedent). | **MEASURED per level (v2 Compact): only subnet/vpc are clean** (gates 0, shorter, X intact, same-col 0); `region`/`account` are structurally safe but reshape columns (same-column edges + wider/taller — the merge crosses container axes so stale `localColumn` no longer forms one layering); `provider` overflows the root stack → overlaps + backward edges. All levels ship (opt-in, default `none`), deep levels **honestly flagged** (dev-API gates/bounds + menu help). **Revisit:** make `region+` CON-12-clean via a post-collapse **leaf-granular global re-rank** (reuse DI-DEB-6's `computeGlobalSeparatedFloor` machinery on the merged set) — deferred. |
...
| **DI-CCM-1** | M6c crossingMin | **The cross-container crossing-min milestone DI-DEB-6 deferred — BUILT (Phase 1) → GO; pays back the +45 % crossings at zero height/width cost.** New pure `terraformPipelineRcllCrossingMin.ts`: `minimizeCrossings` hill-climbs — place a baseline, propose a global **barycenter** sibling order (a child sorts to the mean placed-Y of everything its descendant leaves connect to — Sander/Forster as the PROPOSAL), re-place a clone, accept iff `countPlacedCrossings` (box-center segments over the collapsed cluster edges, the shared `segmentsCross` kernel exported from the diagnostic) **strictly drops** AND containment/overlap ≤ baseline; bounded sweeps + eval budget; empty override ⇒ OFF byte-identical. Threaded as `PlaceCtx.orderOverride`/`LaneContext.orderOverride` honored by all 4 sort sites (forced bands, packed columns, swimlane lanes+leaves, SCC groups) via one `preferredOrder` comparator (override rank primary, model order tiebreak); `crossingMin` wins over `reorder` (`applyRcllToggleGuards` → `ordering-conflict-crossing-min-wins`). Milestone `"M6c"`, `rcllCrossingMin` + `crossingMin*` meta. Planned via `/plan-eng-review`+Codex (verdict "do not build as written" — folded: #1 coordinate fidelity → **rendered gate** not floor proxy; #2 reorder at ALL seams via re-placement; #3 no Forster-sum overclaim; #4 no-dummy soundness — a rendered gate can't optimize phantom long-edge events; #5 measure-driven not pre-pass; **#6 height-budget gate REJECTED by user**). | **The floor-coordinate ordinal gate codex flagged can lower its own count while raising the rendered one** — so the gate reads placed `box.x/box.y`, the same basis as `diagnosePipelineScene`. **X is never a function of order** in this engine (`colByCluster`/`localColumn`/staircase column own it) ⇒ the override moves **only Y** ⇒ CON-12 holds by construction (no re-derivation needed). DI-M6-3 was a no-op because M6 is leaf-only/per-container; M6c reorders the whole sibling tree (lanes + leaves), reaching cross-container crossings. **v2 Compact: rankSeparate+M4 crossings 362→260 (−28 %), Δh=0, Δw=0, 109 moves (proxy 431→317); de-band vpc 268→221 (−18 %), 116 moves; all gates 0, OFF byte-identical, deterministic.** The measured Δh=Δw=0 vindicates the user's height-neutral thesis (codex #6 height gate unnecessary). Locked: `terraformPipelineRcllCrossingMin.test.ts` (12 units) + `terraformPipelineCrossingMin.test.ts` (2 v2 integration) + 2 guard cases. | **No-dummy first** — DEC-5 dummy-node routing escalates only if order alone can't unroute a long edge (the rendered gate dissolves the soundness gap). **Phase 2** (the ~13-file UI/URL/dev-API surface — nested "Cross-container" On/Off dialog control under Ordering, `?crossingMin=1`, dev-API `crossingMin` param + catalog + suppression) BUILT on this win, default OFF / byte-identical (`crossingMin:false` in all three profiles); the `ordering-conflict-crossing-min-wins` guard now also surfaces on the URL/programmatic path. Height/width left **unbounded + reported** (user decision); revisit a budget only if a measured regression appears. Committed `83821086a` (Phase 1 + Phase 2). |
| **DI-CRP-1** | M5r coordRepack | **Coordinated per-column permutation re-pack BUILT behind default-OFF `coordRepack` (requires `straighten`) → GO on the target preset.** New pure `terraformPipelineRepack.ts` (`repackColumns`), applied via `applyCoordRepack` in `terraformPipelineRcllPlacement.ts` at the three straighten sites (packed columns + both lane paths). Per column: brute-force ALL permutations (Heap's) when ≤8 leaves (exact); above 8 a deterministic candidate set (current order + barycenter-sort with stable tiebreak + all single adjacent-swap refinements); coordinate descent over columns in fixed order, ≤8 monotonic sweeps. Each candidate column is placed top-down as a height+gap stack pulled toward each leaf's intra-container-neighbor barycenter, clamped within the FIXED container band. **Never-worse:** adopt only if total intra-container \|ΔY\| **strictly** drops AND CON-3 (band not grown) AND CON-4/5 non-overlap AND intra-container crossing-preserving (`countCrossings` before/after on `sCol!==tCol` within-container edges) AND CON-12 (only Y + within-column order written); else keep the straightened placement. Threaded `coordRepack`/`pipelineCoordRepack`; meta `rcllCoordRepack`/`pipelineCoordRepack` + `pipelineCoordRepackSuppressed`; guard `coord-repack-needs-straighten`. | **NO RNG (CON-8)** — exact brute-force ≤8, deterministic candidate set above. **v2 (target): intra \|ΔY\| 13930→9350 (−32.9 %), median 195→112 (−42.6 %), per-edge net −4580px (22↓/8↑/15=), intra-crossings 4→1, CON-3/4/5/12 = 0 both arms, band growth 0, OFF byte-identical, 25 leaves moved.** `staging-extended-localstack` +0.6 % (one 19-leaf dominant column ⇒ near-no-op + minor drift); `multi-state-expanded`/`localstack` 0 % (no many-small-column containers). **Feasibility-ceiling probe:** the 39–96 % "ceilings" are a MIRAGE (require CON-3 band growth); realistic fixed-band ceiling is **27.2 % on v2**, **~0 %** on single-dominant-column / sparse-intra-edge topologies ⇒ a **v2-class lever, not universal.** | **Two known limitations (honest):** (1) crossing gate is **intra-container only** — cross-container crossings can rise (v2 global 367→371, +4), in-scope since cross-container Y is the closed hull-coord NO-GO; (2) accept gate is **per-container** ⇒ whole-scene intra total can drift (+0.6 % above) — **TODO:** a **global** accept gate (revert all re-packs unless the scene-wide intra total improves) would make it strictly never-worse scene-wide. Revisitable on both counts. |
| **DI-CRP-2** | M5r coordRepack | **Neighbor-barycenter within-column ORDER KEY (the M6-ordering route) — REJECTED as infeasible.** M6 ordering runs **before** coordinate assignment, so it has only a within-column rank, no provisional neighbor Y to barycenter on — the re-pack MUST run post-straighten (DI-CRP-1's site), after Y exists. | The barycenter pull needs a placed neighbor Y, which only exists after coordinate assignment ⇒ ordering-phase placement is the wrong stage. | Reopen only if a provisional-Y estimate is added to the ordering phase. |
| **DI-CRP-3** | M5r coordRepack | **DEC-5 dummy-node chaining in the straightener (`straightenWideSpan`) — BUILT then REVERTED.** Net-negative on v2 (median intra \|ΔY\| 262.5→316, net +1045px span) and inert on sparser presets. | It bought straightness by **growing the band** — exactly the CON-3 violation the `coordRepack` gate forbids; a span win paid in containment is not a real win. | Stays the named DEC-5 escalation only if a band-preserving dummy-routing form is found. |
...
| **Ancillary strips** ("All resources") | the unconnected resources render as per-scope "Unconnected" strips (compound/classic behavior) | **Drawn in RCLL via post-placement reserved bands plus recursive slack allocation.** Earlier export-phase placement measured 90/86 collisions on v2; the built path instead inserts bottom-band rows into the already-placed Dataflow tree, excludes bands from dataflow collectors/model primary counts, parents strips under provider/account/region/VPC hulls, and can widen strips into existing rightward slack only when measured rows shrink. | The invariant is Dataflow X/shape unchanged, root width not increased, placement containment/sibling overlaps zero, and primary Y monotonic. Allocator metrics report real deterministic allocation/drop counts. |
...
| `uncommitted` | **Ancillary** | **RCLL "All resources" reserved bands + recursive slack allocator.** `ancillaryBand` placement role; Dataflow-only baseline placement plus post-placement band insertion; provider/account/region/VPC ancillary scopes; strip rendering + frame parenting via `ancillaryScopeRole`; recursive right-slack wrap-width allocator; final rebuild/gate/drop safety path; UI re-enabled; meta counts + allocator metrics; v2 regression locks no root widening, no collisions, primary X/shape preservation, and allocator determinism. | DI-ANC-4/5 (reserved-band primitive plus measured row-saving allocator over existing slack). | Supersedes DI-ANC-1/3 disabled/deferred state and DI-ANC-4's zero-allocation placeholder; updates §8.1 and §34.2. |
| `uncommitted` | **Ancillary** | **DI-ANC-6 PR1 — read-only band-block diagnostic.** `diagnoseRcllAncillaryBands` dry-runs each row-reduction breakpoint as a candidate FINAL rectangle (clone baseline → widen-only-shorten → reuse gate via the new `classifyAncillaryCandidate`), emitting a per-scope `bandBlockStatus` enum + `currentCeilingWrapWidth`/`bestValidWrapWidth`/`acceptedWrapWidth`/final band box into `rcllAncillaryAllocator.diagnostics`; dev plugin surfaces it under `ancillaryAllocator` (`statusCounts` + `blocked`). Bounded eval (cap 24). Synthetic fixtures pin the `gap-exists-current-algo-missed` and `all-candidates-fail-validation` cases; v2 band geometry byte-identical (additive meta only, allocation unchanged). | DI-ANC-6 (diagnose before fixing: the live ceiling measures overlap vs the tall pre-widen band, hiding movement-free gaps; finding — v2 target is `shared-slack-consumed`, i.e. sibling-scope contention, not the ceiling bug). | Updates §8.1 (block-diagnostic + finding); does not change DI-ANC-5 allocation behavior; scopes PR2 (candidate-final-rectangle fix + shared-slack arbitration) on the evidence. |
...
| `ee824b8b3` | **M5 (direct-leaf rise)** | **Straighten lifts direct resources into open swimlane-axis Y space.** `layoutLanesOnAxis` adds shared lane/leaf occupancy under `straighten=true`; direct leaves use `riseStackY` against already-placed lane boxes before BK, and BK results are clamped through the same occupancy set. X is untouched (`colByCluster`/`columnX` only), no dummy nodes are introduced. UI help now describes "align and lift direct resources." Tests: two synthetic placement regressions + v2 Full-detail compact-ish/no-deband integration; typecheck + RCLL suites green. | DI-M5-7 (coordinate assignment may legally reclaim empty Y bands beside topology lanes; collision/CON-12 gates remain hard). | Amends the earlier "A1 no-op on v2" narrative: the density finding still holds for genuinely occupied columns, but this fixes the local two-phase placement obstacle for direct resources with free column space. |
| `565c75197` | **De-band ladder** | **Generalize subnet de-band → `deBandLevel` hierarchy ladder.** `collapseSubnetsForDeBand` → `collapseTreeForDeBand(node, level)` (collapse at the absorbing parent; cascade is automatic via `collectClusterLeaves`); frame suppression + path truncation + membership rail generalized by depth-rank (`deBandLevelRank`/`topologyRoleDeBandRank`/`DEBAND_LEVELS` in `terraformPipelineLayoutProfiles.ts`); **one rail per dissolved level** (`terraformPipelineSubnetAnnotation.ts`). Surface threaded boolean→enum `pipelineDeBandLevel` end-to-end (`terraformLayoutCore`/`…PlanParsing`/`…PresetImport`/`…SceneApply`/`…ImportSession`/`useTerraformImportDialog`/`TerraformImportDialog`/`TerraformImportPipelineSettings`/`TerraformScenePanel`/`terraformDemoUrlParams`/`TerraformDemoAutoImport`/dev plugin) with `subnetDeBand` kept as the `"subnet"` alias; dialog `<select>` "De-band depth"; URL `?deBandLevel=`; dev-API enum + `resolved.flags.deBandLevel`. Default `none` byte-identical. Tests: profiles + threading (`none`≡off, `vpc` reaches engine, alias≡subnet) + URL + dialog (`<select>` threads + Custom flip) + annotation (rail-per-level + subnet byte-identical) + new `terraformPipelineRcllDeBand.test.ts` (per-level v2 gates + frame suppression + measured deep-level profile); existing `terraformPipelineSubnetDeBand.test.ts` green via alias. typecheck + lint:arch clean. | DI-DEB-7 (collapse at the absorbing parent; depth-rank predicate reused by 4 call sites; boolean kept as alias). | **Generalizes DI-DEB-1..3** (subnet-only → full ladder; not superseded — the subnet level is the alias-preserved special case). **Un-defers §8.2** + generalizes **DEC-11**. **Measured:** subnet/vpc clean; region/account safe-but-reshaping; provider broken on dense graphs — honestly flagged, deep-level CON-12-cleanliness deferred. |
| `83821086a` | **M6c (Phase 1)** | **Container-aware crossing minimization (engine + scorer + tests).** New pure `terraformPipelineRcllCrossingMin.ts` — `minimizeCrossings` measure-driven hill-climb (barycenter proposal → re-place clone → accept iff `countPlacedCrossings` strictly drops ∧ structural gates ≤ baseline; bounded sweeps + eval budget), `barycenterOrder`, `boxByClusterId`, `collapsedEdgesFromFanout`, `countReordered`. Exported the shared `segmentsCross`/`Seg` kernel from `terraformPipelineCollisionDiagnostics.ts` (DRY). Threaded `orderOverride` through `PlaceCtx`/`LaneContext` + the 4 sort sites via one `preferredOrder` comparator; `placementStage` runs the search when `opts.crossingMin`, re-places the accepted order with the columnCompact accumulator, surfaces `crossingMin*` meta. `crossingMin` added to `RcllOptions`/`RcllBuildOptions`; `applyRcllToggleGuards` `ordering-conflict-crossing-min-wins`; milestone `"M6c"`, `rcllCrossingMin` meta. Tests: `terraformPipelineRcllCrossingMin.test.ts` (12), `terraformPipelineCrossingMin.test.ts` (2 v2), 2 guard cases. v2: rankSep+M4 crossings 362→260, de-band vpc 268→221, Δh=Δw=0, gates 0, OFF byte-identical, deterministic; typecheck + RCLL suites green. | DI-CCM-1 (rendered-coordinate measure-driven gate per codex; X order-independent ⇒ Y-only ⇒ CON-12 by construction; no-dummy first; height ungated per user). | **Discharges the DI-DEB-6 +45 % crossings follow-on** (§9.5 item 8 → item 9). Extends **DEC-6** to the shared rendered scorer. **Phase 2** UI surface gated on this win — built in the following change-set. |
| `83821086a` | **M6c (Phase 2)** | **`crossingMin` UI / URL / dev-API surface.** Threaded `pipelineCrossingMin` through the import chain (`RcllLayoutProfileFlags.crossingMin`=false in all profiles; `terraformLayoutCore` context + guard input + post-guard meta echo + `pipelineOrderingConflict` backstop + default resolution; `terraformPlanParsing`/`terraformImportSession`/`terraformPresetImport`/`terraformSceneApply` types + call sites; `useTerraformImportDialog` state + custom-marking wrapper + profile fan-out + import call sites; `TerraformImportDialog`; `TerraformImportPipelineSettings` nested **"Cross-container"** control + 2 `OPTION_HELP` entries; `TerraformDemoAutoImport`; URL `?crossingMin=1`; dev `GET /api/terraform-layout` param + catalog + `crossingMin*` metrics + suppression). Default OFF ⇒ byte-identical. Tests: +2 threading, +1 URL, +1 dialog, +`crossingMin:false` profiles; typecheck + lint:arch clean. | DI-CCM-1 (Phase 2 — surface only; no engine/geometry change). | Mirrors the DI-DEB-6 "Phase-2 UI toggle gated on this surviving review." |
| `8d1a222a2` | **M2-NS (X-axis network simplex)** | **`pipelineColumnPacking:"shorten"` — exact Gansner network-simplex ranker, default OFF, in 3 bisectable PRs.** PR1: extracted shared `applyDepthFloorIfValid` (verify-or-abort + **dual-write** `cluster.depth` → `computeGlobalColumnX` AND `depthResult.depths` → `buildRcllModel`→`lattice.floor`) from the experimental ALAP arm — no-op refactor. PR2: widened the `pipelineColumnPacking` enum to `"shorten"` across the ~13-file import chain + `VALID_COLUMN_PACKING`; the `"shorten"` **bundle** expands (in the `applyRcllToggleGuards({…})` literal) to `{networkSimplexRank, columnCompact}`; meta `rcllNetworkSimplexApplied`/`nsSkipReason`/milestone `M2-NS`; 4th UI radio "Shorten". PR3: pure `computeNetworkSimplexDepths` (feasible tight tree → cut values → pivots to optimum, Bland's-rule leaving-edge = anti-cycle + deterministic, `balance()`; weight = parallel-edge count; per-weakly-connected-component; safety-ceiling→input). Threaded into `preparePipelineLayout` (RCLL builder formerly passed `{}`). Tests: synthetic provable optimum + property + weighting + determinism + dual-write + cyclic + guard + URL + radio + v2 A/B (width −8.4%, gates 0). | DI-NS-1 (exact-to-optimum, never a capped result — a heuristic stalls −10.3%, a sign-bug −7.3%, both still "every edge forward"); DI-NS-2 (dual-write is load-bearing — writing only the map ships green and changes nothing); DI-NS-3 (hand-roll, not elkjs — its NETWORK_SIMPLEX is async/whole-graph, no sync `rank()`). | **Discharges** the only remaining edge-length lever (§ change-log 2026-06-24 X-axis row). **⟦CORRECTED same change-set — DI-NS-4 reverses the eng-review "NS-wins-rankSeparate" guard call.⟧** User repro (full-detail v2, `rankSeparate=1`) showed `shorten` **+149% height**: `rankSeparate`+laneRise is the dominant −60% HEIGHT lever (disjoint lane column ranges → Y-interleave) and NS's min-span re-rank DESTROYS that X-disjointness. NS and rankSeparate CANNOT compose → **rankSeparate WINS** (suppression `rank-floor-conflict-rankseparate-wins-network-simplex`, meta `pipelineNetworkSimplexRankSuppressed`, milestone stays `M8r`); NS only effective when rankSeparate off. Cross-preset validation → TODOS.md TODO-NS-1. |
| `TBD` (backfill-pending) | **M5r (coordRepack)** | **Coordinated per-column permutation re-pack, default OFF, requires `straighten`.** New pure `terraformPipelineRepack.ts` (`repackColumns` — Heap's brute-force ≤8 / deterministic candidate set above 8, coordinate descent ≤8 sweeps, no RNG); `applyCoordRepack` in `terraformPipelineRcllPlacement.ts` at the three straighten sites; never-worse accept gate (intra \|ΔY\| strictly down + CON-3/4/5/12 + intra crossing-preserving). Threaded `coordRepack`/`pipelineCoordRepack` through the import chain + meta `rcllCoordRepack`/`pipelineCoordRepack`/`pipelineCoordRepackSuppressed` + guard `coord-repack-needs-straighten`. v2: intra \|ΔY\| −32.9% total / −42.6% median, intra-crossings 4→1, CON 0, band 0, OFF byte-identical. | DI-CRP-1 (never-worse per-container intra-Y re-pack; exact brute-force ≤8; intra-crossing-only + per-container-gate limitations recorded); DI-CRP-2 (barycenter ORDER KEY rejected — needs placed Y, ordering phase has none); DI-CRP-3 (DEC-5 dummy-chain `straightenWideSpan` BUILT then REVERTED — grew the band). | Builds on **M5 (A1)** `b91a4d77a` (runs post-straighten). Hash is `TBD` until committed; backfill in the following docs commit (DOC-2). |
| `(this commit)` | docs | §34.3 commit-map hash backfill. | — | Filled `8d1a222a2` into the `M2-NS` row above (its own hash was unknowable when written). |
...
| **DI-M5-7** | DI-M5-4 (A1 BK straightener), DI-M4-3 (riseStackY occupancy precedent), DI-M3b-1 (swimlane shared axis), CON-12, Sander compound boxes | amends DI-M5-5 (local obstacle removed; density wall still applies where columns truly overlap) |
| **DI-DEB-7** | DI-DEB-1/2/3 (subnet probe), DI-M3a-16 (shared-axis ⇒ subnet/vpc CON-12-clean), finding "region+ merges across container axes" | generalizes **DI-DEB-1..3** (subnet → full ladder; not superseded — subnet is the alias-preserved special case); revisit reuses **DI-DEB-6** |
| **DI-CCM-1** | **DI-DEB-6** (rankSeparate, source of the +45 % crossings debt this discharges), **DI-M6-3** (the leaf-only/per-container no-op M6c generalizes to the whole sibling tree), the **M5c columnCompact** measure-driven clone-re-place oracle precedent, **DEC-6** (extended to the shared rendered scorer), CON-12 + finding "X is order-independent in this engine" (⇒ Y-only override), codex review (rendered-coordinate gate, no-dummy first) | discharges the **DI-DEB-6** crossings follow-on; extends **DEC-6**; DEC-5 dummy-node routing remains the named escalation (not yet triggered) |
| **DI-CRP-1** | **DI-M5-4** (A1 BK straightener — coordRepack runs post-straighten, needs placed Y), the **M5c columnCompact / DI-CCM-1** measure-driven clone-re-place accept-gate precedent, CON-3 / CON-4/5 / CON-8 / CON-12, finding "the 39–96 % ceilings are a MIRAGE (require band growth); fixed-band ceiling 27.2 % on v2, ~0 % single-dominant-column/sparse-intra" | — (mutually-composes-with **DI-CCM-1** crossingMin + **DI-M6-3**/reorder via the order it inherits; intra-only crossing gate + per-container accept gate are recorded revisitables) |
| **DI-CRP-2** | DI-CRP-1, finding "ordering phase has no placed neighbor Y" | rejected (infeasible at the ordering stage) |
| **DI-CRP-3** | DI-CRP-1, **DEC-5** (dummy-node routing), CON-3 | reverted (band-growth net-negative on v2) |
