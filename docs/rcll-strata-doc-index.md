...
| [`strata-view-w14-browser-felt-cost.md`](./strata-view-w14-browser-felt-cost.md) | Battery | Pre-registered (record only; pending WP1-4) | Browser felt-cost milestone — always-on `withTerraformPlanNodeKeyIndex` scope extension (parsing + skeleton materialization + topology resolvers) + `pipelineFull` worker offload; measures against the frozen W12 Appendix A baseline (15.1s felt import, 13,412ms blocking task, ~8.8s/65% address-key resolution, strata solvers ~26ms, zero worker threads); REPORT-only, no gate/threshold, scene output byte-identical requirement |
