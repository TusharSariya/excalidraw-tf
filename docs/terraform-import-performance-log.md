...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-07-22 | **E11: line-scan DOT edge extraction in `mergeDotAdjacency`.** The queued hypothesis (structuredClone in `namespacePlanForStack`/`attachModuleTree`) was REFUTED by a sub-span decomposition: on the pinned preset `namespacePlanForStack` never runs and the dominant `prep.cache` cost was `graphlibDot.read()` fully parsing a 5.3MB DOT (11,923 nodes / 28,271 edges) only for `mergeDotAdjacency` to read edge (v,w) pairs — 514ms of ~740ms Mac-side prep.cache. Replaced with `extractDotEdgePairsFast` (strict `"src" -> "dst"` one-edge-per-line scan) with a whole-DOT bail to graphlib on ANY deviation, including a `/*` anywhere in the text (adversarial hardening: an edge-shaped line inside a block comment would otherwise become a phantom edge — line scanning has no comment-lexer state). Byte-identity: `sanitizeDotNodeId` strips exactly the chars graphlib unescaping would normalize; edge order = textual order in both paths. DEV counters `dotAdjacencyFastPathHits/Bails` + kill-switch `setDotAdjacencyFastPathDisabledForTest`; new profiler sub-spans `prep.merge.dot`/`prep.nodes.*`/`prep.namespace`/`prep.tfd.overlay`. Known intentional leniency (documented in-code): DOT that graphlib would reject on a malformed non-`->` line now imports instead of throwing — unreachable from `terraform graph` output. | pipeline+semantic | laptop cold import (i3, strata pinned URL) **27.35s**; `prep.cache` 4282 | laptop **23.95s** (5 runs + warmup, spread 4.3%); `prep.cache` 1281 | **−12.4%** felt; `prep.merge.dot` −95% | yes (byte-identical) | `prep.cache` → now `prep.nodes` (~1.2s laptop, `newEdges` dominant = next target) | **Kept + promoted.** Dual adversarial (new policy: Fable-med + Codex gpt-5.6-sol med per role): impl Fable VALID / Codex SUSPECT on the constructed block-comment divergence → fixed `29fcf9856` + regression test; results Fable VALID / Codex SUSPECT on warmup asymmetry → neutralized warm-vs-warm (27.19/27.35 vs 23.2–24.2, no overlap; worst reading −11.0%). Full gauntlet: 3157 passed, 2 failures = pre-existing Mac-only mermaid flake reproduced on E11-free tree, zero `.snap` diffs. Commits `f241c8927`+`29fcf9856` (wt-e11) → strata tip `29fcf9856`. Mac warm prep.cache ~740→~230ms (−69%). |
| 2026-07-22 | **E07: wrap `buildExistingEdges` in the plan-node-key index with incremental key registration.** `buildExistingEdges` (`terraformPlanParsing.tsx`) resolved each edge endpoint via `resolveTerraformPlanNodeKey`, which — outside an active `withTerraformPlanNodeKeyIndex` scope — falls back to its original O(N) `Object.keys(nodes)`+regex scan; `buildExistingEdges` was called without an active index scope, so every edge resolution over N≈3,886 resources paid the full scan. Wrapped the call in `withTerraformPlanNodeKeyIndex` and added incremental key registration so keys discovered mid-build (module-tree attachment, satellite address expansion) get folded into the same index scope instead of falling back or requiring a rebuild. Byte-identical: index-wrapped resolution is provably equivalent to the unwrapped fallback (same `byBareKey`/`byGraphId`/`knownStackIds` index used elsewhere in `prep.cache`), verified by a new equivalence test plus the existing brute-force-twin residual-fallback-scan assertions (now 0 scans on the wrapped path, down from the prior per-edge fallback). | pipeline+semantic | laptop cold import (i3, `staging-multi-state-expanded`) **35.59s** | laptop cold import **26.89s** | **−24.4%** | yes (byte-identical) | `buildExistingEdges` / `prep.cache` | **Kept.** `prep.cache` 13.0s → 4.3s. Full gate green on `strata-overnight-20260717` (cherry-picked from `wt-b` `10ccd1e7d` as `46a5c57f4`): typecheck ✓, `yarn test:app` 295 test files / 3138 tests passed (56 skipped, 1 todo), 0 failures, zero `.snap` diffs. New/extended equivalence test `terraformPlanNodeKeyIndexEquivalence.test.ts` (P2 residual-fallback-scan assertions now assert 0 on the `buildExistingEdges` path). Both adversarial validators returned VALID; confound decomposition found the paint tail adds +20ms (immaterial against the −8.7s win). |
| 2026-07-21 | **E02: thread memoized `nodesByType` index through `buildSatelliteContext` (+ E01 topology memo scope substrate).** Landed the codebase's designed-but-unlanded TODO-3: `buildNodesByTypeIndex` existed but was only built inside `buildAllSatellitePrimaryMappings`; `buildSatelliteContext` never set `ctx.nodesByType`, so every reverse-ref/companion/plugin/height-fn scan on the 427 primary + 304 ancillary cluster builds fell back to an O(all-nodes) `Object.keys(nodes)` walk per kind per address. Added a `WeakMap`-memoized `getMemoizedNodesByTypeIndex(nodes)` (arnIndex pattern, `terraformTopologySatelliteEngine.ts`) and threaded `ctx.nodesByType` into every scanning builder in `buildTopologyPrimarySatelliteBundles` (iam/sg/s3/alb/ecs/eks/ecsCluster/ecsEc2/api/apiVpc/tgw/lambdaPermission/sqs/cloudWatch), into `collectTopologySatelliteAddressesFromRegistry`, into the 12 scanning stack-height fns, and into `buildLambdaPermissionCluster`; `buildAllSatellitePrimaryMappings` now reuses the shared index. Stacks on E01 (`withTopologyLayoutMemoScope` wrap of `pipeline.prep.materialize` + scoped per-(address,kind) memo for `buildSatelliteClusterForKind`), which was NULL alone on main-branch profiling but supplies the memo substrate E02's index sits on top of. Byte-identical, no toggle — order-safety proven empirically (index ON vs OFF byte-equal) since `buildNodesByTypeIndex` preserves `Object.keys(nodes)` insertion order per type bucket. | pipeline | baseline-i1-desktop (i5-13600K, staging-extended-localstack-v2 strata pinned URL) median **8.81s** (8808ms) | e02-desktop (commit `1e95dbbe2`, 3 runs: 7289/7299/7271ms) median **7.29s** (spread 0.4%) | **−17.3%** | yes | `skeleton.resourceRects` (3670→2072ms, −43.6%) | **Kept** (promoted from `perf-loop-exp` onto `strata-overnight-20260717`). Span deltas: `skeleton.resourceRects` 3670→2072 (−1599, −43.6%); `skeleton.satelliteBundles` 925→513 (−412, −44.6%); `strata.ancillary` 2152→1918 (−234, −10.9%); `layout.pipeline` 4286→2688 (−1599); `pipeline.prep.materialize` 1533→168 (−1366, E01 memo now paying off on top of the index). Engagement counters (once-per-build profiler flush): `satellite.nodesByTypeIndex.fallbackScans = 0` (index engaged on every scan site of the real desktop build), `contextsServed` 1991, `pipeline.satMemo` hits 166 / misses 303. Full gate green on `strata-overnight-20260717`: typecheck ✓, `yarn test:app` 301 test files / 3161 tests passed, 0 failures, zero `.snap` diffs. New equivalence tests: `terraformPipelineNodesByTypeIndexEquivalence.test.ts` (3 assertions: pipeline cluster builds byte-identical index ON vs OFF on both full and compact prep and the collect/owner-map path, fallback-scan count many→0 on every asserted path) and `terraformPipelineTopologyMemoScopeEquivalence.test.ts` (E01 memo-on vs -off deep-equality on the pinned preset). Commits on `strata-overnight-20260717`: E01 `543ef9253`, E02 `7a78189d6` (cherry-picked from `perf-loop-exp` `a26ff09d8`/`1e95dbbe2`). |
