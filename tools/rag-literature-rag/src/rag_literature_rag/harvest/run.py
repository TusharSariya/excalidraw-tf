...
from rag_literature_rag.harvest.arxiv import harvest_arxiv
from rag_literature_rag.harvest.arxiv_bulk import harvest_arxiv_category
from rag_literature_rag.harvest.awesome_rag_table import harvest_awesome_rag_table
from rag_literature_rag.harvest.citations import harvest_forward_citations
...
from rag_literature_rag.harvest.checkpoint import (
    clear_bib_state,
    clear_checkpoint,
    discovery_complete,
    load_bib_state,
...
from rag_literature_rag.harvest.parallel import (
    DEFAULT_WORKERS,
    MAX_WORKERS,
    clear_stop,
    request_stop,
...
from rag_literature_rag.harvest.providers import log_provider_summaries
from rag_literature_rag.harvest.retry import retry_unresolved_multi_pass
from rag_literature_rag.harvest.semantic_scholar import harvest_semantic_scholar
from rag_literature_rag.harvest.topic_seeds import harvest_topic_seeds
from rag_literature_rag.harvest.trusted_venues import harvest_trusted_venues
...
def _run_early_stages(manifest, log, *, kw, skip_topic_seeds: bool, dry_run: bool) -> None:
    if not skip_topic_seeds:
        ...
        _run_stage("RAG topic seeds (foundations, GraphRAG, agentic)", log, _harvest_topic_seeds)
        if not dry_run:
            _save_progress(manifest, "topic-seeds", log)

    _run_stage(
        "awesome-generative-ai-guide RAG table",
        log,
        lambda: _merge_items(manifest, harvest_awesome_rag_table(**kw)),
    )
    if not dry_run:
        _save_progress(manifest, "awesome-rag-table", log)

    _run_stage(
        "curated implementer guides",
        log,
    ...
    *,
    ...
    if not pipeline_only:
        sources.append(
            (
                "trusted-venues",
                "Complete trusted venues (ACL/SIGIR via OpenAlex)",
    ...
    if not skip_arxiv:
        ...
        sources.append(
            (
                "arxiv",
                f"arXiv keyword + cs.CL/cs.IR/cs.AI category (max {arxiv_cap} each)",
...
def _apply_pipeline_harvest_defaults(
    ...
    return (
        max_openalex or 2000,
        max_openalex_per_topic or 60,
        max_dblp or 0,
        ...
        target or 1400,
        target_pdfs or 1000,
        retry_passes if retry_passes > 1 else 2,
        bib_passes if bib_passes > 1 else 2,
    )
...
def _apply_deep_harvest_defaults(
    ...
    if deep_harvest:
        return (
            max_openalex or 2000,
            max_openalex_per_topic or 50,
            max_dblp or 200,
            max_semantic_scholar or 400,
            max_arxiv or 300,
            max_bib_dois or 600,
            target or 1400,
            target_pdfs or 1000,
...
def _execute_harvest(
    *,
    ...
    has_early_seeds = any(
        i.source in ("topic-seed", "awesome-rag-table", "curated", "anthropic-cookbook", "implementer-guide")
...
def harvest_options(f):
    ...
    f = click.option(
        "--pipeline-harvest",
        is_flag=True,
        help="Core RAG topics only via OpenAlex pipeline queries.",
    )(f)
...
def harvest_prune_cmd(apply_: bool, yes: bool, sample: int) -> None:
    ...
    """
    from rag_literature_rag.harvest.prune import apply_prune, plan_prune, signal_stats
