...
from graph_layout_rag.ingest import run
from graph_layout_rag.ingest.extract import PageText
from graph_layout_rag.ingest.status import load_status
...
def test_extract_workers_invalid_falls_back(monkeypatch, caplog):
    ...
    monkeypatch.setenv("GRAPH_RAG_EXTRACT_WORKERS", "many")
    try:
        assert run._extract_workers() == 4
        assert "invalid GRAPH_RAG_EXTRACT_WORKERS" in caplog.text
    finally:
...
def test_execute_ingest_flushes_batches_and_checkpoints(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GRAPH_RAG_INGEST_DOC_BATCH", "2")
