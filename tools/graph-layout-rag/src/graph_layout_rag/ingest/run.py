...
from graph_layout_rag.ingest.extract import (
    default_pdf_backend,
...
from graph_layout_rag.ingest.log import setup_ingest_logging
...
from graph_layout_rag.ingest.status import (
    format_duration,
    ingest_status_cmd,
    load_status,
    new_run_status,
...
def _progress_log_every() -> int:
    return max(1, int(os.getenv("GRAPH_RAG_INGEST_PROGRESS_LOG_EVERY", "25")))


def _progress_log_interval_s() -> float:
    return max(1.0, float(os.getenv("GRAPH_RAG_INGEST_PROGRESS_LOG_INTERVAL_S", "30")))


def _extract_workers() -> int:
    raw = os.getenv("GRAPH_RAG_EXTRACT_WORKERS", "4")
    try:
        return max(0, int(raw))
    except ValueError:
        logging.getLogger("graph_layout_rag.ingest").warning(
            "invalid GRAPH_RAG_EXTRACT_WORKERS=%r; using 4", raw
        )
        return 4
...
def _ensure_pdf_backend_available(pdf_backend: str) -> None:
    ...
    try:
        ...
        raise click.ClickException(
            "GRAPH_RAG_PDF_BACKEND=docling but docling is not installed. "
...
def ingest_options(f):
    ...
    f = click.option(
        ...
        default=None,
        help="PDF extraction backend (default GRAPH_RAG_PDF_BACKEND or pymupdf). "
...
from graph_layout_rag.ingest.compare_extract import compare_extract_cmd  # noqa: E402
from graph_layout_rag.ingest.reembed import reembed_cmd  # noqa: E402
