...
Every downstream agent fills its owned rows. ONE owning agent per metric/case is canonical (authority rule); others cite, never recompute. Failure-class ∈ {config, search-space, measurement, objective}. Fixable-by-generic-algorithm ∈ {yes, no, yes-with-tradeoff}. All numbers come from the H0 canonical dual-scoring record (chord AND rendered) at the frozen config — never a hand-moved render.
...
_Convergence = agents agree on case-classifications + fix-verdicts. Disagreement IS the finding — surface it, do not average it away._
